      ...
      <div className="relative bg-white rounded-xl shadow-xl max-w-md w-full mx-4 p-6">
        {submitted ? (
          <div className="text-center py-4">
            ...
            <p className="text-sm text-[var(--ink2)]">Report submitted. Thank you for helping keep our community safe.</p>
          </div>
        ) : (
          <>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-[var(--ink)]">Report {targetType}</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              {REPORT_REASONS.map(r => (
                <label key={r} className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="text-[var(--accent)] focus:ring-[rgba(10,22,40,0.15)]"
                  />
                  <span className="text-sm text-[var(--ink2)]">{r}</span>
                </label>
              ))}
              ...
              {reason === 'Other' && (
                <textarea
                  ...
                  rows={2}
                  maxLength={500}
                  className="w-full px-3 py-2 border border-[var(--line)] rounded-lg text-sm resize-none focus:outline-none focus:ring-2 focus:ring-[rgba(10,22,40,0.15)]"
                />
              )}
