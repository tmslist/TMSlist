  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="w-8 h-8 border-4 border-[var(--line)] border-t-indigo-600 rounded-full animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-3">
        <span className="text-sm font-medium text-[var(--muted)]">Period:</span>
        {[
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-[var(--line)] p-5 shadow-sm">
          <p className="text-xs font-medium text-[var(--muted)] uppercase tracking-wide">Total Reviews</p>
          <p className="text-3xl font-bold text-[var(--ink)] mt-2">{data.totalReviews.toLocaleString()}</p>
          <p className="text-xs text-[var(--muted)] mt-1">in selected period</p>
        </div>
        <div className="bg-white rounded-xl border border-[var(--line)] p-5 shadow-sm">
          <p className="text-xs font-medium text-[var(--muted)] uppercase tracking-wide">Avg Rating</p>
          <div className="flex items-center gap-2 mt-2">
            <p className="text-3xl font-bold text-[var(--ink)]">{data.avgRatingOverall.toFixed(2)}</p>
            <StarRating rating={Math.round(data.avgRatingOverall)} />
          </div>
        </div>
        <div className="bg-white rounded-xl border border-[var(--line)] p-5 shadow-sm">
          <p className="text-xs font-medium text-[var(--muted)] uppercase tracking-wide">Response Rate</p>
          <p className="text-3xl font-bold text-emerald-600 mt-2">{data.responseMetrics.responseRate.toFixed(1)}%</p>
          <p className="text-xs text-[var(--muted)] mt-1">{data.responseMetrics.responded} / {data.responseMetrics.totalReviews}</p>
        </div>
        <div className="bg-white rounded-xl border border-[var(--line)] p-5 shadow-sm">
          <p className="text-xs font-medium text-[var(--muted)] uppercase tracking-wide">NPS Score</p>
      ...
      {/* Rating Trends */}
      <div>
        <h2 className="text-lg font-semibold text-[var(--ink)] mb-4">Rating Trend Over Time</h2>
                ...
                <table className="min-w-full divide-y divide-[var(--line)]">
                  <thead className="bg-[var(--paper2)]">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-left text-xs font-medium text-[var(--muted)] uppercase">Avg Rating</th>
                      <th className="px-4 py-2 text-left text-xs font-medium text-[var(--muted)] uppercase">Reviews</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={trend.month} className="hover:bg-[var(--paper2)]">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-semibold text-[var(--ink)]">{trend.avgRating.toFixed(2)}</span>
                            <StarRating rating={Math.round(trend.avgRating)} />
                          </div>
                        </td>
                        <td className="px-4 py-3 text-sm text-[var(--ink2)]">{trend.count}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-[var(--ink)]">Breakdown</h2>
            ...
            <table className="min-w-full divide-y divide-[var(--line)]">
              <thead className="bg-[var(--paper2)]">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-[var(--muted)] uppercase">Avg Rating</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-[var(--muted)] uppercase">Total</th>
                  <th className="px-4 py-3 text-center text-xs font-medium text-[var(--muted)] uppercase">1</th>
                  <th className="px-4 py-3 text-center text-xs font-medium text-[var(--muted)] uppercase">2</th>
                  <th className="px-4 py-3 text-center text-xs font-medium text-[var(--muted)] uppercase">3</th>
                  <th className="px-4 py-3 text-center text-xs font-medium text-[var(--muted)] uppercase">4</th>
                  <th className="px-4 py-3 text-center text-xs font-medium text-[var(--muted)] uppercase">5</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--line)]">
                {breakdownData.slice(0, 20).map((row, i) => (
                  <tr key={`${row.clinicId}-${i}`} className="hover:bg-[var(--paper2)]">
                    <td className="px-4 py-3 text-sm font-medium text-[var(--ink)] truncate max-w-[200px]">
                      {row.clinicName}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold text-[var(--ink)]">{row.avgRating.toFixed(2)}</span>
                        <StarRating rating={Math.round(row.avgRating)} />
                      </div>
                    </td>
                    <td className="px-4 py-3 text-sm text-[var(--ink2)]">{row.totalReviews}</td>
      ...
      <div>
        <h2 className="text-lg font-semibold text-[var(--ink)] mb-4">Response Metrics</h2>
        <div className="bg-white rounded-xl border border-[var(--line)] p-6 shadow-sm">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold text-[var(--ink)]">{data.responseMetrics.responseRate.toFixed(1)}%</p>
              <p className="text-xs text-[var(--muted)] mt-1">Response Rate</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-[var(--ink)]">{data.responseMetrics.responded}</p>
              <p className="text-xs text-[var(--muted)] mt-1">Reviews Responded</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-[var(--ink)]">{data.responseMetrics.totalReviews}</p>
              <p className="text-xs text-[var(--muted)] mt-1">Total Reviews</p>
            </div>
            <div className="text-center">
              ...
              <p className="text-xs text-[var(--muted)] mt-1">Avg Response Time</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* NPS Score Detail */}
      <div>
        <h2 className="text-lg font-semibold text-[var(--ink)] mb-4">NPS Score Calculation</h2>
        <div className="bg-white rounded-xl border border-[var(--line)] p-6 shadow-sm">
          <div className="mb-6 text-center">
            ...
            <p className="text-sm text-[var(--muted)] mt-2">Net Promoter Score</p>
          </div>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="bg-[var(--paper2)] rounded-lg p-4 text-center border border-[var(--line)]">
              <p className="text-2xl font-bold text-[var(--ink2)]">{data.nps.passiveCount}</p>
              <p className="text-xs text-[var(--muted)] font-medium mt-1">Passives (3-4 star)</p>
              <p className="text-xs text-[var(--muted)] mt-1">
