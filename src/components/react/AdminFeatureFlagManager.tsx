...
const ENVIRONMENTS: Environment[] = [
  { key: 'development', label: 'Development', color: 'bg-[var(--paper2)] text-[var(--ink2)]' },
...
export default function AdminFeatureFlagManager() {
  ...
  const fetchFlags = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const list = Array.isArray(data) ? data : (data.data || data.flags || []);
  ...
  const STATUS_COLORS: Record<string, string> = {
    ...
    beta: 'bg-[rgba(10,22,40,0.1)] text-[var(--ink)]',
    testing: 'bg-amber-100 text-amber-700',
    draft: 'bg-[var(--paper2)] text-[var(--muted)]',
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-[var(--muted)] mt-1">Control feature rollouts across environments and user segments</p>
        </div>
        <button
          onClick={addNew}
      ...
      <div className="mb-6">
        <input
          ...
          className="w-full max-w-md rounded-lg border border-[var(--line)] px-4 py-2.5 text-sm focus:border-[var(--ink2)] focus:ring-[#1E2A3B]"
        />
      </div>
      ...
      <div className="space-y-4">
        {filteredFlags.map(flag => (
          <div key={flag.id} className="bg-white rounded-xl border border-[var(--line)] p-6">
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3">
                  <span className="text-sm font-mono font-bold text-[var(--ink)]">{flag.key}</span>
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${
                      flag.enabled ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200' : 'bg-[var(--paper2)] text-[var(--muted)] hover:bg-[var(--paper2)]'
                    }`}
                  >
                ...
                {flag.description && (
                  <p className="text-sm text-[var(--muted)] mt-1">{flag.description}</p>
                )}
                {/* Environments */}
              ...
              <div className="flex items-center gap-4 ml-6">
                <div className="text-right">
                  <div className="text-sm font-semibold text-[var(--ink)]">{flag.rolloutPercentage}%</div>
                  <div className="text-[11px] text-[var(--muted)]">rollout</div>
                </div>
                <input
                  type="range"
                  min={0}
            ...
            {/* Targeting rules */}
            {flag.targetingRules && (
              <div className="mt-4 pt-4 border-t border-[var(--line)] flex items-center gap-2">
                ...
                <span className="text-xs text-[var(--muted)]">Targeting: <span className="font-medium text-[var(--ink2)]">{flag.targetingRules}</span></span>
              </div>
            )}
                ...
                <div className="flex gap-2">
                  {ENVIRONMENTS.map(env => (
                    <label key={env.key} className="flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm cursor-pointer transition-colors">
                      ...
                      />
                      <span className="font-medium text-[var(--ink2)]">{env.label}</span>
                    </label>
                  ))}
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-[var(--ink2)] mb-2">Rollout Percentage</label>
                <div className="flex items-center gap-4">
                  ...
                  />
                  <span className="text-sm font-bold text-[var(--ink)] w-12 text-right">{editing.rolloutPercentage}%</span>
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-[var(--ink2)] mb-2">User Segment</label>
                <select
