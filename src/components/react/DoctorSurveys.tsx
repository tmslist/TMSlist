...
const SURVEY_TYPE_COLORS: Record<string, string> = {
  phq9: 'bg-[rgba(10,22,40,0.1)] text-[var(--ink)] dark:bg-[var(--paper2)] dark:text-[var(--muted)]',
  gad7: 'bg-[rgba(201,101,74,0.1)] text-[var(--warm)] dark:bg-[rgba(201,101,74,0.40)] dark:text-[var(--muted)]',
  custom: 'bg-[var(--paper2)] text-[var(--ink2)] dark:bg-[var(--ink2)] dark:text-[var(--line)]',
};
    ...
    <div>
      ...
      {showTemplates && (
        <div className="mb-6 bg-white dark:bg-[var(--ink)] border border-[var(--line)] dark:border-[var(--ink2)] rounded-xl shadow-sm p-6">
          <h3 className="text-base font-semibold text-[var(--ink)] dark:text-white mb-4">Survey Templates</h3>
          <div className="space-y-4">
            {SURVEY_TEMPLATES.map(template => (
              <div key={template.name} className="border border-[var(--line)] dark:border-[var(--ink2)] rounded-xl p-4">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <h4 className="text-sm font-semibold text-[var(--ink)] dark:text-white">{template.name}</h4>
                    <p className="text-xs text-[var(--muted)] dark:text-[var(--muted)] mt-0.5">{template.description}</p>
                  </div>
                ...
                <div className="mb-3">
                  <p className="text-xs font-medium text-[var(--muted)] dark:text-[var(--muted)] mb-1">Questions ({template.questions.length}):</p>
                  <ol className="text-xs text-[var(--ink2)] dark:text-[var(--line)] space-y-0.5 list-decimal list-inside">
                    {template.questions.slice(0, 4).map((q, i) => (
                      <li key={i}>{q}</li>
                    ))}
                    {template.questions.length > 4 && (
                      <li className="text-[var(--muted)] dark:text-[var(--muted)] italic">+{template.questions.length - 4} more</li>
                    )}
                  </ol>
                </div>
                <button
                  onClick={() => applyTemplate(template)}
                  className="text-xs px-3 py-1.5 rounded-lg font-medium text-[var(--ink)] dark:text-[var(--ink2)] bg-[var(--paper2)] dark:bg-[var(--paper2)] hover:bg-[rgba(10,22,40,0.1)] dark:hover:bg-[var(--paper2)] transition-colors"
                >
                  Use Template
                </button>
              </div>
      ...
      {showForm && (
        <div className="mb-6 bg-white dark:bg-[var(--ink)] border border-[var(--line)] dark:border-[var(--ink2)] rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold text-[var(--ink)] dark:text-white">Create New Survey</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-[var(--ink2)] dark:text-[var(--line)] mb-1">Survey Name *</label>
                <input
                  type="text"
                  value={form.name}
              ...
              <div>
                <label className="block text-xs font-medium text-[var(--ink2)] dark:text-[var(--line)] mb-1">Survey Type *</label>
                <select
      ...
      {viewingSurvey && (
        <div className="mb-6 bg-white dark:bg-[var(--ink)] border border-[var(--line)] dark:border-[var(--ink2)] rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-base font-semibold text-[var(--ink)] dark:text-white">{viewingSurvey.name}</h3>
            ...
            <button onClick={() => { setViewingSurvey(null); setViewingResponses([]); }} className="text-[var(--muted)] hover:text-[var(--ink2)] dark:hover:text-[var(--line)]">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          ...
          {loadingResponses ? (
            <div className="flex items-center justify-center py-8">
              <div className="w-6 h-6 border-3 border-[var(--line)] border-t-blue-600 rounded-full animate-spin" />
            </div>
          ) : viewingResponses.length === 0 ? (
            <div className="text-center py-6">
              <p className="text-[var(--muted)] dark:text-[var(--muted)] text-sm">No responses recorded yet.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {viewingResponses.map((response, i) => (
                <div key={response.id} className="border border-[var(--line)] dark:border-[var(--ink2)] rounded-lg p-3">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-sm font-medium text-[var(--ink)] dark:text-white">{response.questionKey}</p>
                      <p className="text-sm text-[var(--ink2)] dark:text-[var(--line)] mt-1">{response.answer}</p>
                    </div>
                    {response.score !== null && (
                      <span className="text-xs font-semibold text-[var(--muted)] dark:text-[var(--muted)]">Score: {response.score}</span>
                    )}
                  </div>
                </div>
              ))}
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:items-center justify-between mb-6">
        ...
        {activeTab === 'list' && (
          <select
            ...
            className="px-3 py-2 rounded-lg text-sm font-medium text-[var(--ink2)] dark:text-[var(--line)] bg-white dark:bg-[var(--ink2)] border border-[var(--line)] dark:border-[var(--ink2)] focus:ring-2 focus:ring-[rgba(10,22,40,0.2)]"
          >
        ...
        <div>
          {filteredSurveys.length === 0 ? (
            <div className="bg-white dark:bg-[var(--ink)] border border-[var(--line)] dark:border-[var(--ink2)] rounded-xl p-8 text-center">
              <p className="text-[var(--muted)] dark:text-[var(--muted)] text-sm">No surveys found. Create your first survey above.</p>
            </div>
          ) : (
            <div className="bg-white dark:bg-[var(--ink)] border border-[var(--line)] dark:border-[var(--ink2)] rounded-xl shadow-sm overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-[var(--paper2)] dark:bg-[var(--ink2)] border-b border-[var(--line)] dark:border-[var(--ink2)]">
                  <tr>
                    <th className="text-left px-4 py-3 font-medium text-[var(--ink2)] dark:text-[var(--line)]">Survey Name</th>
                    <th className="text-left px-4 py-3 font-medium text-[var(--ink2)] dark:text-[var(--line)]">Type</th>
                    <th className="text-left px-4 py-3 font-medium text-[var(--ink2)] dark:text-[var(--line)]">Status</th>
                    <th className="text-left px-4 py-3 font-medium text-[var(--ink2)] dark:text-[var(--line)]">Score</th>
                ...
                <tbody>
                  {filteredSurveys.map(survey => {
                    ...
                    return (
                      <tr key={survey.id} className="border-b border-[var(--line)] dark:border-[var(--ink2)] last:border-0 hover:bg-[var(--paper2)] dark:hover:bg-[var(--paper2)]">
                        <td className="px-4 py-3">
                          <p className="font-medium text-[var(--ink)] dark:text-white">{survey.name}</p>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          {survey.completedAt ? (
                            ...
                            <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-[var(--paper2)] dark:bg-[var(--ink2)] text-[var(--ink2)] dark:text-[var(--muted)]">Pending</span>
                          )}
                        </td>
                        <td className="px-4 py-3">
                          {survey.score !== null ? (
                            <div className="flex items-center gap-2">
                              <span className="font-semibold text-[var(--ink)] dark:text-white">{survey.score}</span>
                              {severity && (
                            ...
                            <span className="text-[var(--muted)] dark:text-[var(--muted)]">—</span>
                          )}
                        </td>
                        <td className="px-4 py-3 text-[var(--muted)] dark:text-[var(--muted)] text-xs">{new Date(survey.createdAt).toLocaleDateString()}</td>
                        <td className="px-4 py-3">
                          <button
                            onClick={() => viewSurveyResponses(survey)}
                            className="text-xs text-[var(--ink)] dark:text-[var(--ink2)] hover:underline"
                          >
                            View Responses
                          </button>
                        </td>
      ...
      {activeTab === 'analytics' && (
        <div>
          {Object.keys(surveyStats).length === 0 ? (
            <div className="bg-white dark:bg-[var(--ink)] border border-[var(--line)] dark:border-[var(--ink2)] rounded-xl p-8 text-center">
              <p className="text-[var(--muted)] dark:text-[var(--muted)] text-sm">No survey data for analytics yet.</p>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {Object.entries(surveyStats).map(([type, stats]) => {
                  ...
                  return (
                    <div key={type} className="bg-white dark:bg-[var(--ink)] border border-[var(--line)] dark:border-[var(--ink2)] rounded-xl p-5 shadow-sm">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-xs text-[var(--muted)] dark:text-[var(--muted)]">{stats.total} total</span>
                      </div>
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          <span className="text-[var(--muted)] dark:text-[var(--muted)]">Completed</span>
                          <span className="font-semibold text-[var(--ink)] dark:text-white">{stats.completed} ({stats.total > 0 ? Math.round(stats.completed / stats.total * 100) : 0}%)</span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span className="text-[var(--muted)] dark:text-[var(--muted)]">Avg Score</span>
                      ...
                      {def && (
                        <p className="text-xs text-[var(--muted)] dark:text-[var(--muted)] mt-3 italic">{def.description}</p>
                      )}
                    </div>
                  );
                })}
              ...
              {/* Response Rate Bar */}
              <div className="bg-white dark:bg-[var(--ink)] border border-[var(--line)] dark:border-[var(--ink2)] rounded-xl p-6 shadow-sm">
                <h4 className="text-sm font-semibold text-[var(--ink)] dark:text-white mb-4">Overall Response Rate</h4>
                {(() => {
                  ...
                  return (
                    <div>
                      <div className="flex justify-between text-xs text-[var(--muted)] dark:text-[var(--muted)] mb-2">
                        <span>{totalCompleted} completed of {totalSurveys} distributed</span>
                        <span className="font-semibold">{rate.toFixed(0)}%</span>
                      </div>
                      <div className="w-full bg-[var(--paper2)] dark:bg-[var(--ink2)] rounded-full h-3">
                        <div className="h-3 rounded-full bg-[var(--ink2)] transition-all" style={{ width: `${rate}%` }} />
                      </div>
                    </div>
                  );
                })()}
              ...
              {['phq9', 'gad7'].filter(type => surveyStats[type]).length > 0 && (
                <div className="bg-white dark:bg-[var(--ink)] border border-[var(--line)] dark:border-[var(--ink2)] rounded-xl p-6 shadow-sm">
                  <h4 className="text-sm font-semibold text-[var(--ink)] dark:text-white mb-4">Score Distribution</h4>
                  <div className="space-y-3">
                    {(['phq9', 'gad7'] as const).filter(type => surveyStats[type]).map(type => {
                      ...
                      return (
                        <div key={type}>
                          <p className="text-xs font-medium text-[var(--muted)] dark:text-[var(--muted)] mb-2 uppercase tracking-wide">{SURVEY_TYPE_LABELS[type]}</p>
                          <div className="space-y-1.5">
                            {Object.entries(ranges).map(([range, count]) => (
                              <div key={range} className="flex items-center gap-3">
                                <span className="text-xs text-[var(--ink2)] dark:text-[var(--line)] w-20 shrink-0">{range}</span>
                                <div className="flex-1 bg-[var(--paper2)] dark:bg-[var(--ink2)] rounded-full h-5 relative">
                                  <div
                                    className={`h-5 rounded-full ${rangeColors[range] || 'bg-[var(--ink2)]'}`}
                                    style={{ width: `${(count / typeSurveys.length) * 100}%` }}
                                  />
                                  <span className="absolute right-2 top-0.5 text-xs font-medium text-[var(--ink2)] dark:text-[var(--line)]">{count}</span>
                                </div>
                              </div>
                            ))}
                          </div>
