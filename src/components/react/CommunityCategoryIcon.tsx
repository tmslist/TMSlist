...
export default function CommunityCategoryIcon({ icon, color, size = 'md' }: CommunityCategoryIconProps) {
  const sizeClass = size === 'lg' ? 'w-10 h-10 p-2' : size === 'sm' ? 'w-6 h-6 p-1' : 'w-8 h-8 p-1.5';
  const colorClass = COLOR_CLASSES[color || 'violet'] || COLOR_CLASSES.violet;
  const svg = ICONS[icon || 'brain'] || ICONS.brain;

  return (
    <div className={`${sizeClass} ${colorClass} rounded-lg flex items-center justify-center shrink-0`}>
      {svg}
    </div>
  );
}

// Static rendering helper for Astro pages (returns icon key to SVG path data)
export function getCategoryIconName(icon: string | null): string {
  return icon || 'brain';
}
