...
const DEFAULT_CLINIC_TEMPLATES: BadgeTemplate[] = [
  ...
  { id: 'fast_responder', type: 'clinic', key: 'fast_responder', name: 'Fast Responder', description: 'Responds to leads within 2 hours on average', icon: 'bolt', color: '#0A1628', criteria: { type: 'automatic', metric: 'response_time', threshold: 120, description: 'Avg response time < 2 hours' }, active: true, createdAt: '' },
    ...
    <div className="grid grid-cols-6 gap-2">
      {ICON_OPTIONS.map(icon => (
        <button
          ...
          className={`p-2 rounded-lg border text-center transition-colors ${value === icon.key ? 'border-[rgba(201,101,74,0.2)] bg-[rgba(201,101,74,0.06)]' : 'border-[var(--line)] hover:border-[var(--line)]'}`}
          title={icon.label}
        >
...
function Modal({ open, onClose, title, children }: { open: boolean; onClose: () => void; title: string; children: React.ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-[var(--ink2)] rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--line)] dark:border-[var(--ink2)]">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-[var(--paper2)] dark:hover:bg-[var(--ink2)] text-[var(--muted)] dark:text-[var(--muted)] hover:text-[var(--ink2)] dark:hover:text-white">
...
export default function AdminBadgeEditor() {
  ...
  const badgeColors = ['#0A1628', '#8b5cf6', '#ec4899', '#ef4444', '#f59e0b', '#10b981', '#3b82f6', '#14b8a6', '#f97316', '#84cc16'];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-xl font-semibold text-[var(--ink)] dark:text-white">Badge Editor</h2>
          <p className="text-sm text-[var(--muted)] dark:text-[var(--muted)] mt-0.5">Configure badge templates and award criteria</p>
        </div>
        <button onClick={fetchData} className="px-3 py-2 bg-white dark:bg-[var(--ink2)] border border-[var(--line)] dark:border-[var(--ink2)] rounded-lg text-sm font-medium text-[var(--ink2)] dark:text-[var(--line)] hover:bg-[var(--paper2)] dark:hover:bg-[var(--ink2)] transition-colors flex items-center gap-2">
      ...
      <div className="bg-white dark:bg-[var(--ink2)] rounded-xl shadow-sm border border-[var(--line)] dark:border-[var(--ink2)]">
        <div className="border-b border-[var(--line)] dark:border-[var(--ink2)]">
          <nav className="flex gap-1 px-4">
            <button onClick={() => setTab('clinic')} className={`px-4 py-3 text-sm font-medium border-b-2 -mb-px transition-colors ${tab === 'clinic' ? 'border-[rgba(201,101,74,0.2)] text-[var(--warm)]' : 'border-transparent text-[var(--muted)] dark:text-[var(--muted)] hover:text-[var(--ink2)] dark:hover:text-[var(--line)]'}`}>
              Clinic Badges
            </button>
            <button onClick={() => setTab('doctor')} className={`px-4 py-3 text-sm font-medium border-b-2 -mb-px transition-colors ${tab === 'doctor' ? 'border-[rgba(201,101,74,0.2)] text-[var(--warm)]' : 'border-transparent text-[var(--muted)] dark:text-[var(--muted)] hover:text-[var(--ink2)] dark:hover:text-[var(--line)]'}`}>
              Doctor Badges
            </button>
          </nav>
        </div>
          ...
          {loading ? (
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {currentTemplates.map(tpl => (
                <div key={tpl.id} className={`p-4 border rounded-xl transition-colors ${tpl.active ? 'border-[var(--line)] dark:border-[var(--ink2)] bg-white dark:bg-[var(--ink2)]' : 'border-[var(--line)] dark:border-[var(--ink2)] bg-[var(--paper2)] dark:bg-[var(--ink)] opacity-60'}`}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h4 className="text-sm font-semibold text-[var(--ink)] dark:text-white truncate">{tpl.name}</h4>
                        <span className={`px-1.5 py-0.5 text-[10px] font-medium rounded ${tpl.active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-[var(--paper2)] text-[var(--muted)] dark:bg-[var(--ink2)] dark:text-[var(--muted)]'}`}>
                          {tpl.active ? 'Active' : 'Inactive'}
                        </span>
                      </div>
                      <p className="text-xs text-[var(--muted)] dark:text-[var(--muted)] mt-0.5">{tpl.description}</p>
                      <div className="mt-2">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium ${tpl.criteria.type === 'automatic' ? 'bg-[rgba(10,22,40,0.1)] text-[var(--ink)] dark:bg-[var(--paper2)] dark:text-[var(--ink2)]' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'}`}>
                          {tpl.criteria.type === 'automatic' ? 'Auto' : 'Manual'}
                        </span>
                        {tpl.criteria.metric && (
                          <span className="ml-1 text-[10px] text-[var(--muted)] dark:text-[var(--muted)]">{tpl.criteria.description || `${tpl.criteria.metric} ${tpl.criteria.threshold}`}</span>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <button onClick={() => handleToggle(tpl)} className="p-1.5 rounded-lg hover:bg-[var(--paper2)] dark:hover:bg-[var(--ink2)] text-[var(--muted)] dark:text-[var(--muted)]" title={tpl.active ? 'Deactivate' : 'Activate'}>
                        {tpl.active ? (
                      ...
                      <button onClick={() => openModal(tpl)} className="p-1.5 rounded-lg hover:bg-[var(--paper2)] dark:hover:bg-[var(--ink2)] text-[var(--muted)] dark:text-[var(--muted)]">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                      </button>
                      <button onClick={() => handleDelete(tpl)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-[var(--muted)] dark:text-[var(--muted)] hover:text-red-600 dark:hover:text-red-400">
      ...
      <div className="bg-white dark:bg-[var(--ink2)] rounded-xl shadow-sm border border-[var(--line)] dark:border-[var(--ink2)]">
        <div className="px-6 py-4 border-b border-[var(--line)] dark:border-[var(--ink2)]">
          <h3 className="text-sm font-semibold text-[var(--ink)] dark:text-white">Badge Distribution</h3>
          <p className="text-xs text-[var(--muted)] dark:text-[var(--muted)] mt-0.5">Track which badges have been awarded</p>
        </div>
        <div className="p-6">
          {awards.length === 0 ? (
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-[var(--paper2)] dark:bg-[var(--ink)] border-b border-[var(--line)] dark:border-[var(--ink2)]">
                    <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] dark:text-[var(--muted)] uppercase">Recipient</th>
                    <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] dark:text-[var(--muted)] uppercase">Badge</th>
                    <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] dark:text-[var(--muted)] uppercase">Awarded</th>
                    <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] dark:text-[var(--muted)] uppercase">By</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[var(--line)] dark:divide-[var(--line)]">
                  {awards.slice(0, 20).map(award => (
                    <tr key={award.id} className="hover:bg-[var(--paper2)] dark:hover:bg-[var(--paper2)] transition-colors">
                      <td className="px-4 py-3 text-sm font-medium text-[var(--ink)] dark:text-white">{award.entityName}</td>
                      ...
                      <td className="px-4 py-3 text-xs text-[var(--muted)] dark:text-[var(--muted)]">{award.awardedBy || 'System'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editingTemplate ? 'Edit Badge Template' : 'New Badge Template'}>
        <div className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[var(--ink2)] dark:text-[var(--line)] mb-1">Badge Key *</label>
              <input
                type="text"
                value={formKey}
                ...
                className="w-full rounded-lg border border-[var(--line)] dark:border-[var(--ink2)] px-3 py-2 text-sm bg-white dark:bg-[var(--ink)] focus:ring-2 focus:ring-[rgba(10,22,40,0.2)] disabled:opacity-50"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-[var(--ink2)] dark:text-[var(--line)] mb-1">Display Name *</label>
              <input
                type="text"
                value={formName}
                onChange={e => setFormName(e.target.value)}
                placeholder="My Badge"
                className="w-full rounded-lg border border-[var(--line)] dark:border-[var(--ink2)] px-3 py-2 text-sm bg-white dark:bg-[var(--ink)] focus:ring-2 focus:ring-[rgba(10,22,40,0.2)]"
              />
            </div>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--ink2)] dark:text-[var(--line)] mb-1">Icon</label>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-12 h-10 rounded border border-[var(--line)] dark:border-[var(--ink2)] cursor-pointer"
              />
              <input
                type="text"
                value={formColor}
                onChange={e => setFormColor(e.target.value)}
                className="flex-1 rounded-lg border border-[var(--line)] dark:border-[var(--ink2)] px-3 py-2 text-sm font-mono bg-white dark:bg-[var(--ink)]"
              />
              <div className="flex gap-1">
                {badgeColors.map(c => (
                  <button
                    ...
                    className="w-6 h-6 rounded border border-[var(--line)] dark:border-[var(--ink2)] hover:scale-110 transition-transform"
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--ink2)] dark:text-[var(--line)] mb-1">Award Criteria</label>
            <div className="flex gap-4 mb-3">
              <label className="flex items-center gap-2 text-sm">
                ...
                />
                <span className="text-[var(--ink2)] dark:text-[var(--line)]">Manual (admin awards)</span>
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="radio"
                  ...
                  className="text-[var(--warm)]"
                />
                <span className="text-[var(--ink2)] dark:text-[var(--line)]">Automatic (system awards)</span>
              </label>
            </div>
            ...
            {formCriteriaType === 'automatic' && (
              <div className="space-y-3 p-3 bg-[var(--paper2)] dark:bg-[var(--ink)] rounded-lg">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full rounded-lg border border-[var(--line)] dark:border-[var(--ink2)] px-3 py-1.5 text-sm bg-white dark:bg-[var(--ink2)]"
                    >
                ...
                <div>
                  <label className="block text-xs text-[var(--muted)] dark:text-[var(--muted)] mb-1">Criteria Description</label>
                  <input
                    type="text"
