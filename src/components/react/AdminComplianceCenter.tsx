  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--ink)]">Compliance Center</h1>
          <p className="text-[var(--muted)] mt-1 text-sm">HIPAA compliance, audit trail, and data retention management</p>
        </div>
        <button onClick={() => setShowReportModal(true)} className="inline-flex items-center gap-2 px-4 py-2.5 bg-[var(--ink2)] text-white text-sm font-medium rounded-xl hover:bg-[var(--ink2)] transition-colors shadow-sm">
        ...
        <div className="space-y-3">
          {[
            ...
            <button key={opt.type} onClick={() => handleExportReport(opt.type)}
              className="w-full flex items-center justify-between p-4 bg-[var(--paper2)] hover:bg-[var(--paper2)] rounded-xl border border-[var(--line)] transition-colors text-left">
              <div>
                <p className="text-sm font-medium text-[var(--ink)]">{opt.label}</p>
                <p className="text-xs text-[var(--muted)] mt-0.5">{opt.sub}</p>
              </div>
              <svg className="w-4 h-4 text-[var(--muted)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
            </button>
          ))}
        </div>
      </Modal>
...
function HipaaTab({ items, compliant, partial, nonCompliant, score }: {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default: return { label: 'N/A', bg: 'bg-[var(--paper2)]', text: 'text-[var(--muted)]', border: 'border-[var(--line)]', icon: '?' };
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* Score banner */}
      <div className="flex items-center gap-6 p-5 bg-gradient-to-r from-[rgba(201,101,74,0.1)] to-[rgba(10,22,40,0.08)] rounded-xl border border-[var(--line)]">
        <div className="relative w-20 h-20 shrink-0">
          <svg viewBox="0 0 36 36" className="w-full h-full -rotate-90">
            <path d="M18 2.0845a 15.9155 15.9155 0 0 1 0 31.831a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="#E6EAF0" strokeWidth="3" />
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-lg font-bold text-[var(--ink)]">{score}%</span>
          </div>
        </div>
        <div>
          <h3 className="text-lg font-semibold text-[var(--ink)]">HIPAA Compliance Score</h3>
          <p className="text-sm text-[var(--muted)] mt-1">Based on {items.length} requirements across 5 categories</p>
      ...
      <div className="space-y-3">
        {HIPAA_CATEGORIES.map(cat => {
          ...
          return (
            <div key={cat} className="border border-[var(--line)] rounded-xl overflow-hidden">
              <button onClick={() => setExpandedCategory(isOpen ? '' : cat)} className="w-full flex items-center justify-between p-4 bg-[var(--paper2)] hover:bg-[var(--paper2)] transition-colors">
                <div className="flex items-center gap-3">
                  <span className="text-sm font-semibold text-[var(--ink)]">{cat}</span>
                  <span className="text-xs text-[var(--muted)]">({catCompliant}/{catItems.length} compliant)</span>
                </div>
                      ...
                      <div key={item.id} className="p-4">
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1">
                            ...
                            {item.notes && <p className="text-xs text-[var(--muted)] mt-1 italic">Note: {item.notes}</p>}
                            <p className="text-xs text-[var(--line)] mt-1">Last checked: {formatDate(item.lastChecked)}</p>
                          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select value={actionFilter} onChange={e => onActionFilter(e.target.value)} className="text-sm border border-[var(--line)] rounded-lg px-3 py-2 focus:ring-2 focus:ring-[rgba(10,22,40,0.2)] bg-white">
          <option value="">All Actions</option>
          {allActions.map(a => <option key={a} value={a}>{a}</option>)}
        </select>
        <select value={entityFilter} onChange={e => onEntityFilter(e.target.value)} className="text-sm border border-[var(--line)] rounded-lg px-3 py-2 focus:ring-2 focus:ring-[rgba(10,22,40,0.2)] bg-white">
          <option value="">All Entities</option>
          {allEntities.map(e => <option key={e} value={e}>{e}</option>)}
        </select>
        <input type="date" value={dateFrom} onChange={e => onDateFrom(e.target.value)} className="text-sm border border-[var(--line)] rounded-lg px-3 py-2 focus:ring-2 focus:ring-[rgba(10,22,40,0.2)]" />
        <input type="date" value={dateTo} onChange={e => onDateTo(e.target.value)} className="text-sm border border-[var(--line)] rounded-lg px-3 py-2 focus:ring-2 focus:ring-[rgba(10,22,40,0.2)]" />
        {hasFilters && <button onClick={onClear} className="text-xs text-[var(--muted)] hover:text-[var(--ink2)] px-2">Clear filters</button>}
      </div>

      <div className="text-xs text-[var(--muted)] mb-2">{total} entries</div>
      ...
      <div className="overflow-x-auto rounded-lg border border-[var(--line)]">
        <table className="min-w-full">
          <thead className="bg-[var(--paper2)]">
            <tr>
              <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Time</th>
              <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">User</th>
              <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Action</th>
              <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Entity</th>
              <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Details</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[var(--line)]">
            {entries.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-[var(--muted)]">No audit entries found</td></tr>
            ) : entries.map(entry => (
              <tr key={entry.id} className="hover:bg-[var(--paper2)] transition-colors">
                <td className="px-4 py-3 text-xs text-[var(--muted)] whitespace-nowrap" title={formatDate(entry.createdAt)}>{formatRelativeTime(entry.createdAt)}</td>
                <td className="px-4 py-3 text-sm text-[var(--ink)]">{entry.userEmail || 'System'}</td>
                <td className="px-4 py-3">
                  <span className="text-xs font-medium text-[var(--warm)] bg-[rgba(201,101,74,0.06)] px-2 py-1 rounded-md">{entry.action}</span>
                </td>
                <td className="px-4 py-3 text-xs text-[var(--ink2)]">
                  <span className="bg-[var(--paper2)] px-2 py-0.5 rounded">{entry.entityType}</span>
                  {entry.entityId && <span className="ml-1 font-mono text-[var(--muted)]">{entry.entityId.slice(0, 8)}</span>}
                </td>
                <td className="px-4 py-3 text-xs text-[var(--muted)] max-w-[200px] truncate">{entry.details ? JSON.stringify(entry.details) : '-'}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {pages > 1 && (
        <div className="flex items-center justify-between px-1">
          <span className="text-sm text-[var(--muted)]">Page {page + 1} of {pages}</span>
          <div className="flex items-center gap-2">
            <button onClick={() => onPageChange(Math.max(0, page - 1))} disabled={page === 0}
              className="px-3 py-1.5 text-sm border border-[var(--line)] rounded-lg hover:bg-[var(--paper2)] disabled:opacity-40 transition-colors">Previous</button>
            <button onClick={() => onPageChange(Math.min(pages - 1, page + 1))} disabled={page >= pages - 1}
              className="px-3 py-1.5 text-sm border border-[var(--line)] rounded-lg hover:bg-[var(--paper2)] disabled:opacity-40 transition-colors">Next</button>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <button onClick={onAdd} className="px-4 py-2 bg-[var(--ink2)] text-white text-sm font-medium rounded-xl hover:bg-[var(--ink2)] transition-colors flex items-center gap-2">
      ...
      {data.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-lg border border-[var(--line)]">
          <table className="min-w-full">
            <thead className="bg-[var(--paper2)]">
              <tr>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Entity Type</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Retention Period</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Description</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Status</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Created</th>
            ...
            <tbody className="divide-y divide-[var(--line)]">
              {data.map(policy => (
                <tr key={policy.id} className="hover:bg-[var(--paper2)] transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-[var(--muted)] max-w-xs truncate">{policy.description || '-'}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${policy.active ? 'bg-emerald-100 text-emerald-700' : 'bg-[var(--paper2)] text-[var(--muted)]'}`}>
                      {policy.active ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-xs text-[var(--muted)] whitespace-nowrap">{formatDate(policy.createdAt)}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => onEdit(policy)} className="p-1.5 text-[var(--muted)] hover:text-[var(--warm)] rounded-lg hover:bg-[rgba(201,101,74,0.06)]" title="Edit">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                      </button>
                      <button onClick={() => onDelete(policy.id)} className="p-1.5 text-[var(--muted)] hover:text-red-600 rounded-lg hover:bg-red-50" title="Delete">
    ...
    <div className="space-y-4">
      ...
      {data.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {data.map(doc => (
            <div key={doc.id} className={`p-5 rounded-xl border ${doc.isActive ? 'border-emerald-200 bg-emerald-50/30' : 'border-[var(--line)]'}`}>
              <div className="flex items-start justify-between gap-3 mb-3">
                <div>
                  <div className="flex items-center gap-2">
                    <h4 className="text-sm font-semibold text-[var(--ink)]">{DOC_TYPE_LABELS[doc.type] || doc.type}</h4>
                    {doc.isActive && <span className="text-xs font-semibold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-full">Active</span>}
                  </div>
                  <p className="text-xs text-[var(--muted)] mt-0.5">Version {doc.version} &middot; {formatDate(doc.createdAt)}</p>
                </div>
                <button onClick={() => onEdit(doc)} className="shrink-0 p-1.5 text-[var(--muted)] hover:text-[var(--warm)] rounded-lg hover:bg-[rgba(201,101,74,0.06)]">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                </button>
              </div>
              <div className="text-sm text-[var(--ink2)] max-h-24 overflow-y-auto bg-white rounded-lg p-3 border border-[var(--line)]">
  ...
  return (
    <div className="space-y-4">
      ...
      <label className="flex items-center gap-2 text-sm text-[var(--ink2)]">
        <input type="checkbox" checked={active} onChange={e => setActive(e.target.checked)} className="w-4 h-4 rounded border-[var(--line)] text-[var(--warm)]" />
        Active policy
      </label>
      <div className="flex justify-end gap-3 pt-4 border-t border-[var(--line)]">
        ...
        <button onClick={handleSubmit} className="px-4 py-2.5 text-sm font-medium text-white bg-[var(--ink2)] rounded-xl hover:bg-[var(--ink2)]">Save</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <select value={type} onChange={e => setType(e.target.value)} className="w-full text-sm border border-[var(--line)] rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-[rgba(10,22,40,0.2)] focus:border-[rgba(201,101,74,0.2)] outline-none bg-white">
      ...
      <div>
        ...
        <textarea value={content} onChange={e => setContent(e.target.value)} rows={12} placeholder="Enter document content..."
          className="w-full text-sm border border-[var(--line)] rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-[rgba(10,22,40,0.2)] focus:border-[rgba(201,101,74,0.2)] outline-none resize-none font-mono" />
      </div>
      <label className="flex items-center gap-2 text-sm text-[var(--ink2)]">
        <input type="checkbox" checked={isActive} onChange={e => setIsActive(e.target.checked)} className="w-4 h-4 rounded border-[var(--line)] text-[var(--warm)]" />
