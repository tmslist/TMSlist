...
function getSeverityColor(severity: string): { bg: string; text: string; border: string } {
  switch (severity) {
    ...
    default: return { bg: 'bg-[var(--paper2)]', text: 'text-[var(--ink)]', border: 'border-[var(--line)]' };
  }
}
    ...
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-scale-in">
          <h3 className="text-base font-semibold text-[var(--ink)] mb-2">{title}</h3>
          <p className="text-sm text-[var(--muted)] mb-5">{message}</p>
          <div className="flex justify-end gap-3">
            <button onClick={onCancel} className="px-4 py-2 text-sm font-medium text-[var(--ink2)] bg-white border border-[var(--line)] rounded-xl hover:bg-[var(--paper2)]">Cancel</button>
            <button onClick={onConfirm} className={`px-4 py-2 text-sm font-medium text-white rounded-xl hover:opacity-90 ${danger ? 'bg-red-600' : 'bg-[var(--ink2)]'}`}>
              {confirmLabel}
            </button>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--ink)]">Security Dashboard</h1>
          <p className="text-[var(--muted)] mt-1 text-sm">Monitor auth security, sessions, and access controls</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              { label: 'Active Sessions', value: stats.activeSessions, color: 'text-[var(--ink)]' },
              { label: 'Alerts 24h', value: stats.suspicious24h, color: 'text-amber-600' },
            ].map(stat => (
              <div key={stat.label} className="bg-white border border-[var(--line)] rounded-lg px-4 py-2 text-center">
                <p className={`text-xl font-bold ${stat.color}`}>{stat.value}</p>
                <p className="text-xs text-[var(--muted)] mt-0.5">{stat.label}</p>
              </div>
            ))}
          </div>
          <button onClick={fetchData} className="p-2 text-[var(--muted)] hover:text-[var(--warm)] hover:bg-[rgba(201,101,74,0.06)] rounded-lg transition-colors" title="Refresh">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-[var(--ink2)]">Failed Login Attempts</h2>
        <span className="text-xs text-[var(--muted)]">{data.length} attempts</span>
      </div>
      {data.length === 0 ? (
        <div className="text-center py-12 text-[var(--muted)]">
          <svg className="w-10 h-10 mx-auto mb-3 text-[var(--line)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-[var(--line)]">
            <thead>
              <tr className="bg-[var(--paper2)]">
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Time</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Email</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">IP Address</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Device</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Reason</th>
            ...
            <tbody className="divide-y divide-[var(--line)]">
              {data.map(entry => (
                <tr key={entry.id} className="hover:bg-[var(--paper2)] transition-colors">
                  <td className="px-4 py-3 text-xs text-[var(--muted)] whitespace-nowrap" title={formatFullTime(entry.attemptedAt)}>
                    {formatRelativeTime(entry.attemptedAt)}
                  </td>
                  <td className="px-4 py-3 text-sm font-medium text-[var(--ink)]">{entry.email}</td>
                  <td className="px-4 py-3 text-xs font-mono text-[var(--ink2)]">{entry.ipAddress}</td>
                  <td className="px-4 py-3 text-xs text-[var(--muted)] max-w-[200px] truncate">{entry.userAgent || 'Unknown'}</td>
...
function SessionsTab({ data, onLogout }: { data: ActiveSession[]; onLogout: (s: ActiveSession) => void }) {
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-[var(--ink2)]">Active Sessions</h2>
        <span className="text-xs text-[var(--muted)]">{data.length} active</span>
      </div>
      {data.length === 0 ? (
        <div className="text-center py-12 text-[var(--muted)]">
          <svg className="w-10 h-10 mx-auto mb-3 text-[var(--line)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="space-y-3">
          {data.map(session => {
            ...
            return (
              <div key={session.id} className="flex items-center justify-between p-4 bg-[var(--paper2)] rounded-xl border border-[var(--line)] hover:bg-[var(--paper2)] transition-colors">
                <div className="flex items-start gap-4 flex-1 min-w-0">
                  <div className="w-10 h-10 bg-[rgba(201,101,74,0.1)] rounded-full flex items-center justify-center shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-[var(--ink)]">{session.userEmail}</span>
                      {session.userName && <span className="text-xs text-[var(--muted)]">({session.userName})</span>}
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className="text-xs text-[var(--muted)]">Expires {formatRelativeTime(session.expiresAt)}</span>
...
function SuspiciousTab({ data }: { data: SuspiciousEvent[] }) {
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-[var(--ink2)]">Suspicious Events</h2>
        <span className="text-xs text-[var(--muted)]">{data.length} events</span>
      </div>
      {data.length === 0 ? (
        <div className="text-center py-12 text-[var(--muted)]">
          <svg className="w-10 h-10 mx-auto mb-3 text-[var(--line)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-sm font-medium text-[var(--ink)]">{event.action}</span>
                    </div>
                    <div className="text-xs text-[var(--ink2)] flex items-center gap-3 flex-wrap">
                      {event.userEmail && <span>User: {event.userEmail}</span>}
                      {event.ipAddress && <span>IP: {event.ipAddress}</span>}
                      <span>{formatRelativeTime(event.createdAt)}</span>
                    </div>
                    {event.metadata && (
                      <pre className="mt-2 text-xs text-[var(--muted)] bg-white/60 rounded-lg p-2 max-h-24 overflow-auto font-mono">
...
function IpRulesTab({ data, onAdd, onEdit, onDelete }: {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-[var(--ink2)]">IP Access Rules</h2>
        <button onClick={onAdd} className="px-3 py-1.5 text-xs font-medium bg-[var(--ink2)] text-white rounded-lg hover:bg-[var(--ink2)] transition-colors flex items-center gap-1.5">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-[var(--line)]">
            <thead>
              <tr className="bg-[var(--paper2)]">
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">IP Address</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Type</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Reason</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Expires</th>
            ...
            <tbody className="divide-y divide-[var(--line)]">
              {data.map(rule => (
                <tr key={rule.id} className="hover:bg-[var(--paper2)] transition-colors">
                  <td className="px-4 py-3 text-xs font-mono font-medium text-[var(--ink)]">{rule.ipAddress}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-xs text-[var(--ink2)]">{rule.reason}</td>
                  <td className="px-4 py-3 text-xs text-[var(--muted)]">{rule.expiresAt ? formatRelativeTime(rule.expiresAt) : 'Never'}</td>
                  <td className="px-4 py-3 text-xs text-[var(--muted)] whitespace-nowrap">{formatRelativeTime(rule.createdAt)}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => onEdit(rule)} className="p-1.5 text-[var(--muted)] hover:text-[var(--warm)] rounded-lg hover:bg-[rgba(201,101,74,0.06)]" title="Edit">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                      </button>
                      <button onClick={() => onDelete(rule.id)} className="p-1.5 text-[var(--muted)] hover:text-red-600 rounded-lg hover:bg-red-50" title="Delete">
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-scale-in">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-lg font-semibold text-[var(--ink)]">{existing ? 'Edit IP Rule' : 'Add IP Rule'}</h2>
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-[var(--ink2)] mb-1">IP Address *</label>
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Rule Type</label>
              <div className="flex gap-2">
                {(['block', 'whitelist'] as const).map(t => (
                  <button key={t} onClick={() => setType(t)} className={`flex-1 px-4 py-2.5 text-sm font-medium rounded-xl border transition-colors ${
                    type === t ? (t === 'block' ? 'bg-red-50 border-red-300 text-red-700' : 'bg-emerald-50 border-emerald-300 text-emerald-700') : 'bg-[var(--paper2)] border-[var(--line)] text-[var(--ink2)]'
                  }`}>
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Reason</label>
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Expires (optional)</label>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          { label: '2FA Not Enabled', value: disabled, color: 'text-[var(--ink2)]', bg: 'bg-[var(--paper2)] border-[var(--line)]' },
          ...
          <table className="min-w-full divide-y divide-[var(--line)]">
            <thead>
              <tr className="bg-[var(--paper2)]">
                ...
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">2FA Status</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Devices</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Last Login</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--muted)] uppercase">Failed Attempts</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[var(--line)]">
              {data.map(user => (
                <tr key={user.email} className="hover:bg-[var(--paper2)] transition-colors">
                  <td className="px-4 py-3 text-sm font-medium text-[var(--ink)]">{user.email}</td>
                  <td className="px-4 py-3">
                    {user.totpEnabled ? (
                      ...
                      <span className="inline-flex items-center gap-1 text-xs font-semibold text-[var(--muted)] bg-[var(--paper2)] px-2.5 py-1 rounded-full">
                        Not enabled
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-sm text-[var(--ink2)]">{user.registeredDevices}</td>
                  <td className="px-4 py-3 text-xs text-[var(--muted)] whitespace-nowrap">{user.lastLoginAt ? formatRelativeTime(user.lastLoginAt) : 'Never'}</td>
                  <td className="px-4 py-3">
