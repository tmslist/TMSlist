  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-xl font-semibold text-[var(--ink)]">SLA Monitor</h2>
          <p className="text-sm text-[var(--muted)] mt-0.5">Service level compliance and uptime tracking</p>
        </div>
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-[rgba(201,101,74,0.06)] rounded-xl p-4">
                  <p className="text-xs font-medium text-[var(--warm)] uppercase">Uptime</p>
                  <p className="text-2xl font-bold text-[var(--warm)] mt-1">{uptimeDisplay}</p>
                  <p className="text-xs text-[var(--warm)]">Current period</p>
                </div>
                <div className="bg-[var(--paper2)] rounded-xl p-4">
                  <p className="text-xs font-medium text-[var(--ink)] uppercase">Incidents</p>
                  <p className="text-2xl font-bold text-[var(--ink)] mt-1">{totalIncidents}</p>
                  <p className="text-xs text-[var(--ink2)]">Total recorded</p>
                </div>
              ...
              <div className="border border-[var(--line)] rounded-xl p-6">
                <h4 className="text-sm font-semibold text-[var(--ink)] mb-4">Uptime Gauge</h4>
                <div className="relative w-48 h-48 mx-auto">
                  <svg viewBox="0 0 100 100" className="w-full h-full">
                    <circle cx="50" cy="50" r="40" fill="none" stroke="#E6EAF0" strokeWidth="8" />
                    <circle cx="50" cy="50" r="40" fill="none" stroke="#0A1628" strokeWidth="8"
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center">
                      <p className="text-2xl font-bold text-[var(--ink)]">{uptimeDisplay}</p>
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="bg-[var(--paper2)] border-b border-[var(--line)]">
                        <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] uppercase">Period</th>
                        <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] uppercase">Uptime</th>
                        <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] uppercase">Incidents</th>
                        <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] uppercase">Downtime</th>
                        <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] uppercase">Affected Users</th>
                        <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] uppercase">Status</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[var(--line)]">
                      {metrics.slice(0, 20).map(m => (
                        <tr key={m.id} className="hover:bg-[var(--paper2)] transition-colors">
                          <td className="px-4 py-3 text-sm font-medium text-[var(--ink)]">{m.period}</td>
                          <td className="px-4 py-3 text-sm text-[var(--ink2)]">{parseFloat(m.uptimePercent).toFixed(3)}%</td>
                          <td className="px-4 py-3 text-sm text-[var(--ink2)]">{m.incidentCount}</td>
                          <td className="px-4 py-3 text-sm text-[var(--ink2)]">{m.totalDowntimeMinutes}m</td>
                          <td className="px-4 py-3 text-sm text-[var(--ink2)]">{m.affectedUsers}</td>
                          <td className="px-4 py-3">
                            <span className={`px-2 py-1 text-xs font-medium rounded ${COMPLIANCE_COLORS[m.complianceStatus] || 'bg-[var(--paper2)] text-[var(--ink2)]'}`}>
                              {m.complianceStatus}
                            </span>
                          </td>
                        </tr>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-[var(--paper2)] rounded-xl p-4">
                  <p className="text-xs font-medium text-[var(--ink)] uppercase">Avg Affected Users</p>
                  ...
                  <p className="text-xs text-[var(--ink2)]">Per incident</p>
                </div>
              </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-[var(--paper2)] border-b border-[var(--line)]">
                      ...
                      <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] uppercase">Downtime</th>
                      <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] uppercase">Affected Users</th>
                      <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] uppercase">Within RTO</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[var(--line)]">
                    {metrics.slice(0, 20).map(m => (
            ...
            <div className="space-y-6">
              <div className="flex gap-3">
                <button onClick={() => exportReport('csv')} disabled={exporting} className="px-4 py-2 bg-[var(--ink2)] text-white text-sm font-medium rounded-lg hover:bg-[var(--ink2)] disabled:opacity-50 transition-colors flex items-center gap-2">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                  Export CSV
                </button>
                <button onClick={() => exportReport('pdf')} disabled={exporting} className="px-4 py-2 border border-[var(--line)] text-sm font-medium text-[var(--ink2)] rounded-lg hover:bg-[var(--paper2)] transition-colors flex items-center gap-2">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="border border-[var(--line)] rounded-xl p-6">
                <h4 className="text-sm font-semibold text-[var(--ink)] mb-4">SLA Report Summary</h4>
                <dl className="space-y-3">
                  <div className="flex justify-between">
                    <dt className="text-sm text-[var(--muted)]">Report Generated</dt>
                    <dd className="text-sm font-medium text-[var(--ink)]">{new Date().toLocaleString()}</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-sm text-[var(--muted)]">Periods Covered</dt>
                    <dd className="text-sm font-medium text-[var(--ink)]">{metrics.length}</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-sm text-[var(--muted)]">Total Incidents</dt>
                    <dd className="text-sm font-medium text-[var(--ink)]">{totalIncidents}</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-sm text-[var(--muted)]">Total Downtime</dt>
                    <dd className="text-sm font-medium text-[var(--ink)]">{totalDowntime} minutes</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-sm text-[var(--muted)]">SLA Compliance Rate</dt>
                    <dd className="text-sm font-medium text-emerald-600">{metrics.length > 0 ? ((metCount / metrics.length) * 100).toFixed(1) : 0}%</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-sm text-[var(--muted)]">Average Uptime</dt>
