...
const STATUS_COLORS: Record<string, string> = {
  ...
  running: 'bg-[rgba(10,22,40,0.1)] text-[var(--ink)]',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-xl font-semibold text-[var(--ink)]">Backup Manager</h2>
          <p className="text-sm text-[var(--muted)] mt-0.5">Manage backups, schedules, and restore points</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={triggerManualBackup} disabled={runningBackup}
            className="px-4 py-2 bg-[var(--ink2)] text-white text-sm font-medium rounded-lg hover:bg-[var(--ink2)] disabled:opacity-50 transition-colors flex items-center gap-2">
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="space-y-4">
              {backups.length === 0 ? (
                <div className="text-center py-12 text-[var(--muted)]">No backups recorded. Run a manual backup to get started.</div>
              ) : (
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="bg-[var(--paper2)] border-b border-[var(--line)]">
                        ...
                        <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] uppercase">Size</th>
                    ...
                    <tbody className="divide-y divide-[var(--line)]">
                      {backups.map(b => (
                        <tr key={b.id} className="hover:bg-[var(--paper2)] transition-colors">
                          <td className="px-4 py-3 text-sm font-medium text-[var(--ink)]">{b.name}</td>
                          <td className="px-4 py-3 text-sm text-[var(--ink2)] capitalize">{b.type}</td>
                          <td className="px-4 py-3">
                            <span className={`px-2 py-1 text-xs font-medium rounded ${STATUS_COLORS[b.status] || 'bg-[var(--paper2)] text-[var(--ink2)]'}`}>{b.status}</span>
                          </td>
                          <td className="px-4 py-3 text-sm text-[var(--ink2)]">{formatBytes(b.sizeBytes)}</td>
                          <td className="px-4 py-3 text-xs text-[var(--muted)]">{formatDateTime(b.createdAt)}</td>
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <button onClick={() => openRestoreModal(b)} className="px-3 py-1.5 text-xs font-medium border border-[var(--line)] rounded-lg hover:bg-[var(--paper2)] transition-colors">Restore</button>
                            </div>
                          </td>
                        </tr>
                      ))}
            ...
            <div className="space-y-4">
              <div className="flex justify-end">
                <button onClick={() => openScheduleModal()} className="px-4 py-2 bg-[var(--ink2)] text-white text-sm font-medium rounded-lg hover:bg-[var(--ink2)] transition-colors flex items-center gap-2">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {schedules.length === 0 ? (
                <div className="text-center py-12 text-[var(--muted)]">No backup schedules configured.</div>
              ) : (
                <div className="space-y-3">
                  {schedules.map(s => (
                    <div key={s.id} className="border border-[var(--line)] rounded-xl p-4">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <h4 className="text-sm font-semibold text-[var(--ink)]">{s.name}</h4>
                            <span className="px-2 py-0.5 bg-[var(--paper2)] text-[var(--ink2)] text-xs font-medium rounded capitalize">{s.frequency}</span>
                            {!s.enabled && <span className="px-2 py-0.5 bg-amber-100 text-amber-700 text-xs font-medium rounded">Paused</span>}
                          </div>
                          <p className="text-xs text-[var(--muted)]">Retention: {s.retentionCount} copies | Type: {s.type}</p>
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => toggleSchedule(s)} className={`w-10 h-5 rounded-full transition-colors relative ${s.enabled ? 'bg-[var(--ink2)]' : 'bg-[var(--line)]'}`}>
                          ...
                          <button onClick={() => openScheduleModal(s)} className="p-1.5 rounded-lg hover:bg-[var(--paper2)] text-[var(--muted)]">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                            </svg>
                          </button>
                          <button onClick={() => deleteSchedule(s)} className="p-1.5 rounded-lg hover:bg-red-50 text-[var(--muted)] hover:text-red-600">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="space-y-4">
              <div className="text-center py-12 text-[var(--muted)]">
                <svg className="w-12 h-12 mx-auto mb-3 text-[var(--line)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <Modal open={showScheduleModal} onClose={() => setShowScheduleModal(false)} title={editingSchedule ? 'Edit Schedule' : 'Add Schedule'}>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Schedule Name *</label>
            <input type="text" value={scheduleName} onChange={e => setScheduleName(e.target.value)} placeholder="Daily Database Backup" className="w-full rounded-lg border border-[var(--line)] px-3 py-2 text-sm focus:ring-2 focus:ring-[rgba(10,22,40,0.2)] focus:border-[rgba(201,101,74,0.2)]" />
          </div>
          <div>
            <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Backup Type</label>
            <select value={scheduleType} onChange={e => setScheduleType(e.target.value)} className="w-full rounded-lg border border-[var(--line)] px-3 py-2 text-sm bg-white focus:ring-2 focus:ring-[rgba(10,22,40,0.2)] focus:border-[rgba(201,101,74,0.2)]">
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Frequency</label>
            <select value={scheduleFrequency} onChange={e => setScheduleFrequency(e.target.value)} className="w-full rounded-lg border border-[var(--line)] px-3 py-2 text-sm bg-white focus:ring-2 focus:ring-[rgba(10,22,40,0.2)] focus:border-[rgba(201,101,74,0.2)]">
              {FREQUENCY_OPTIONS.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Retention Count</label>
            <input type="number" value={scheduleRetention} onChange={e => setScheduleRetention(e.target.value)} min="1" max="100" className="w-full rounded-lg border border-[var(--line)] px-3 py-2 text-sm focus:ring-2 focus:ring-[rgba(10,22,40,0.2)] focus:border-[rgba(201,101,74,0.2)]" />
          </div>
          <div>
            <label className="flex items-center gap-2 text-sm text-[var(--ink2)]">
              <input type="checkbox" checked={scheduleEnabled} onChange={e => setScheduleEnabled(e.target.checked)} className="w-4 h-4 rounded border-[var(--line)] text-[var(--warm)] focus:ring-[rgba(10,22,40,0.2)]" />
              Enabled
            </label>
          </div>
          <div className="flex justify-end gap-3 pt-4 border-t border-[var(--line)]">
            <button onClick={() => setShowScheduleModal(false)} className="px-4 py-2 border border-[var(--line)] rounded-lg text-sm font-medium text-[var(--ink2)] hover:bg-[var(--paper2)]">Cancel</button>
            <button onClick={saveSchedule} disabled={saving} className="px-4 py-2 bg-[var(--ink2)] text-white text-sm font-medium rounded-lg hover:bg-[var(--ink2)] disabled:opacity-50">{saving ? 'Saving...' : 'Save'}</button>
          </div>
        </div>
      </Modal>
      ...
      <Modal open={showRestoreModal} onClose={() => setShowRestoreModal(false)} title="Restore Backup">
        {selectedBackup && (
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-[var(--line)]">
              <button onClick={() => setShowRestoreModal(false)} className="px-4 py-2 border border-[var(--line)] rounded-lg text-sm font-medium text-[var(--ink2)] hover:bg-[var(--paper2)]">Cancel</button>
