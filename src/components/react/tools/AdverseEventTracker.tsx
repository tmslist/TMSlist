...
const severityConfig: Record<Severity, { label: string; bg: string; text: string }> = {
  ...
  4: { label: 'Severe', bg: 'bg-[rgba(201,101,74,0.1)]', text: 'text-[var(--warm)]' },
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Events', value: events.length, Icon: ClipboardIcon, color: 'text-[var(--ink2)]' },
      ...
      {events.length > 0 ? (
        <div className="space-y-3">
          <h3 className="text-sm font-bold text-[var(--ink2)]">Event Log</h3>
          {events.map(event => {
            const typeInfo = EVENT_TYPES.find(t => t.label === event.type);
            const sev = severityConfig[event.severity];
            return (
              <div key={event.id} className={`rounded-xl border p-5 ${event.resolved ? 'border-[var(--line)] opacity-60' : 'border-[var(--line)]'}`}>
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3">
                    {typeInfo ? (() => { const IconC = typeInfo.icon; return <IconC className="text-[var(--ink2)] mt-0.5" size={20} />; })() : <ClipboardIcon className="text-[var(--ink2)] mt-0.5" size={20} />}
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-sm font-bold text-[var(--ink)]">{event.type}</p>
                      ...
                      <p className="text-xs text-[var(--muted)]">{event.date}{event.protocol ? ` · ${event.protocol}` : ''}</p>
                      <p className="text-sm text-[var(--ink2)] mt-2">{event.description}</p>
                      {event.reportedToFDA && (
        ...
        <div className="bg-[var(--paper2)] rounded-xl p-8 text-center border border-[var(--line)]">
          <ShieldIcon className="text-[var(--muted)] mx-auto mb-2" size={32} />
          <p className="text-sm font-bold text-[var(--muted)]">No adverse events logged</p>
          <p className="text-xs text-[var(--muted)] mt-1">Use this tracker to document any unusual events during TMS treatment.</p>
        </div>
      )}
      ...
      {!showForm ? (
        ...
        <div className="bg-white rounded-2xl border border-[var(--line)] p-6 space-y-4">
          <h3 className="text-sm font-bold text-[var(--ink2)]">Log New Event</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-[var(--muted)] uppercase tracking-wider mb-1.5">Event Type</label>
              <select
                value={newEvent.type || ''}
                onChange={e => setNewEvent(prev => ({ ...prev, type: e.target.value }))}
                className="w-full px-4 py-3 border border-[var(--line)] rounded-xl text-[var(--ink)] bg-white focus:ring-2 focus:ring-[rgba(10,22,40,0.15)] outline-none"
              >
            ...
            <div>
              <label className="block text-xs font-semibold text-[var(--muted)] uppercase tracking-wider mb-1.5">Severity (1-5)</label>
              <input
                type="number"
                min="1"
                max="5"
            ...
            <div>
              <label className="block text-xs font-semibold text-[var(--muted)] uppercase tracking-wider mb-1.5">Date</label>
              <input
                type="date"
                value={newEvent.date || ''}
            ...
            <div>
              <label className="block text-xs font-semibold text-[var(--muted)] uppercase tracking-wider mb-1.5">Protocol in use</label>
              <input
                type="text"
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-3 border border-[var(--line)] rounded-xl text-[var(--ink)] bg-white focus:ring-2 focus:ring-[rgba(10,22,40,0.15)] outline-none resize-none text-sm"
            />
          </div>
          <div className="flex items-center gap-4">
            <label className="flex items-center gap-2 text-sm text-[var(--ink2)] cursor-pointer">
              <input
                type="checkbox"
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="px-6 py-3 bg-[var(--paper2)] text-[var(--ink2)] font-semibold rounded-xl hover:bg-[var(--paper2)] transition-colors"
            >
              Cancel
            </button>
          </div>
