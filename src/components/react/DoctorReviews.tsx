...
const SENTIMENT_COLORS: Record<string, string> = {
  ...
  neutral: 'bg-[var(--paper2)] text-[var(--ink2)]',
      ...
      {reviews.length === 0 ? (
        ...
        <div className="space-y-4">
          {reviews.map(review => {
            ...
            return (
              <div key={review.id} className="bg-white rounded-xl border border-[var(--line)] p-5 shadow-sm">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-semibold text-[var(--ink)]">{review.userName}</span>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setFlaggingReview(flaggingReview === review.id ? null : review.id)} className="text-xs text-[var(--muted)] hover:text-red-600 px-2 py-1 border border-[var(--line)] rounded-lg">Flag</button>
                  </div>
                </div>
                {review.title && <p className="font-medium text-[var(--ink)] mb-2">{review.title}</p>}
                <p className="text-[var(--ink2)] text-sm mb-4">{review.body}</p>

                {/* Owner response */}
                {review.ownerResponse ? (
                  <div className="bg-[var(--paper2)] border border-[var(--line)] rounded-lg p-3">
                    <p className="text-xs font-semibold text-[var(--ink)] mb-1">Your Response</p>
                    <p className="text-sm text-[var(--ink2)]">{review.ownerResponse}</p>
                  </div>
                  ...
                  <div className="mt-3">
                    ...
                    />
                    <div className="flex gap-2">
                      <button onClick={() => submitResponse(review.id)} className="px-4 py-2 rounded-lg text-sm font-medium text-white bg-[var(--ink)] hover:bg-[var(--ink)]">Submit</button>
                      <button onClick={() => setRespondingTo(null)} className="px-4 py-2 rounded-lg text-sm font-medium text-[var(--ink2)] border border-[var(--line)] hover:bg-[var(--paper2)]">Cancel</button>
                    </div>
                  </div>
                ) : (
                  <button onClick={() => setRespondingTo(review.id)} className="mt-2 text-sm text-[var(--ink)] hover:text-[var(--ink)] font-medium">Respond to this review</button>
                )}

                {/* Flag reason modal */}
                {flaggingReview === review.id && (
                  <div className="mt-3 p-3 bg-[var(--paper2)] border border-[var(--line)] rounded-lg">
                    <p className="text-sm font-medium text-[var(--ink2)] mb-2">Flag reason</p>
                    <textarea
                      value={flagReason}
                      onChange={e => setFlagReason(e.target.value)}
                      rows={2}
                      className="w-full px-3 py-2 border border-[var(--line)] rounded-lg text-sm focus:outline-none focus:border-[rgba(10,22,40,0.2)] mb-2"
