...
const COLOR_MAP = {
  indigo: { bg: 'bg-[rgba(201,101,74,0.1)]', border: 'border-[var(--line)]', hover: 'hover:bg-[rgba(201,101,74,0.1)]', text: 'text-[var(--accent)]' },
  amber: { bg: 'bg-amber-100', border: 'border-amber-100', hover: 'hover:bg-amber-100', text: 'text-amber-500' },
  rose: { bg: 'bg-[rgba(201,101,74,0.1)]', border: 'border-[var(--line)]', hover: 'hover:bg-[rgba(201,101,74,0.1)]', text: 'text-[var(--accent2)]' },
  violet: { bg: 'bg-[rgba(10,22,40,0.08)]', border: 'border-[rgba(10,22,40,0.1)]', hover: 'hover:bg-[rgba(10,22,40,0.08)]', text: 'text-[var(--accent2)]' },
} as const;
...
export default function OnboardingToast() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[200] w-80 max-w-[calc(100vw-3rem)]">
      <div className="bg-white dark:bg-[var(--ink2)] rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.2)] ring-1 ring-[rgba(10,22,40,0.1)] dark:ring-[rgba(10,22,40,0.1)] overflow-hidden animate-in fade-in slide-in-from-bottom-4">
        {/* Header */}
        <div className="bg-gradient-to-r from-[#0A1628] to-[#0A1628] p-4 text-white">
        ...
        <div className="p-4">
          {step === 'welcome' && nextAction && (
            <div>
              <p className="text-sm text-[var(--ink2)] dark:text-[var(--line)] mb-3">
                {doneCount === 0
              ...
              <a
                ...
                className={`flex items-center gap-3 p-3 rounded-xl border-2 ${COLOR_MAP[nextAction.color as ColorKey]?.border || 'border-[var(--line)]'} ${COLOR_MAP[nextAction.color as ColorKey]?.bg || 'bg-[rgba(201,101,74,0.06)]'} ${COLOR_MAP[nextAction.color as ColorKey]?.hover || 'hover:bg-[rgba(201,101,74,0.1)]'} transition-all group`}
              >
                <div className={`w-10 h-10 rounded-xl ${COLOR_MAP[nextAction.color as ColorKey]?.bg || 'bg-[rgba(201,101,74,0.1)]'} flex items-center justify-center shrink-0`}>
                  {nextAction.icon === 'user' && (
                    <svg className={`w-5 h-5 ${COLOR_MAP[nextAction.color as ColorKey]?.text || 'text-[var(--accent)]'}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" /></svg>
                  )}
                  {nextAction.icon === 'star' && (
                    <svg className={`w-5 h-5 ${COLOR_MAP[nextAction.color as ColorKey]?.text || 'text-[var(--accent2)]'}`} fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10.868 2.884c-.321-.772-1.415-.772-1.736 0l-1.83 4.401-4.753.381c-.833.067-1.171 1.107-.536 1.651l3.62 3.102-1.106 4.637c-.194.813.691 1.456 1.405 1.02L10 15.591l4.069 2.485c.713.436 1.598-.207 1.404-1.02l-1.106-4.637 3.62-3.102c.635-.544.297-1.584-.536-1.65l-4.752-.382-1.831-4.401z" clipRule="evenodd" /></svg>
                  )}
                  {nextAction.icon === 'heart' && (
                    <svg className={`w-5 h-5 ${COLOR_MAP[nextAction.color as ColorKey]?.text || 'text-[var(--accent2)]'}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
                  )}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-[var(--ink)]">{nextAction.label}</p>
                  <p className="text-xs text-[var(--muted)] mt-0.5">{nextAction.sublabel}</p>
                </div>
                <svg className="w-5 h-5 text-[var(--accent2)] group-hover:text-[var(--accent)] transition-colors" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
              ...
              {doneCount < total - 1 && (
                <button
                  ...
                  className="w-full mt-2 text-xs text-[var(--accent)] hover:text-[var(--accent)] font-medium text-center"
                >
          ...
          {step === 'checklist' && (
            <div className="space-y-2">
                  ...
                  <a
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl transition-all group ${
                      isDone
                        ...
                        : 'bg-[var(--paper2)] hover:bg-[rgba(201,101,74,0.06)] border border-transparent hover:border-[var(--line)]'
                    }`}
                  >
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                      isDone ? 'bg-emerald-100' : 'bg-white border border-[var(--line)] group-hover:border-[var(--line)]'
                    }`}>
                      {isDone ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-medium ${isDone ? 'text-emerald-700 line-through' : 'text-[var(--ink)]'}`}>{item.label}</p>
                    </div>
                    {!isDone && (
                      <svg className="w-4 h-4 text-[var(--accent2)] group-hover:text-[var(--accent)]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                      </svg>
                    )}
                  </a>
              ...
              <button
                ...
                className="w-full mt-1 text-xs text-[var(--accent)] hover:text-[var(--accent)] font-medium text-center"
              >
                ← Back
              </button>
            </div>
