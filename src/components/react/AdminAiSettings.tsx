...
const MODEL_PROVIDER_COLORS: Record<string, string> = {
  ...
  Google: 'bg-[rgba(10,22,40,0.1)] text-[var(--ink)]',
  Meta: 'bg-[rgba(10,22,40,0.1)] text-[var(--ink)]',
};
...
function ModelCard({ model, onEdit }: { model: AiModel; onEdit: (m: AiModel) => void }) {
  const colorClass = MODEL_PROVIDER_COLORS[model.provider] ?? 'bg-[var(--paper2)] text-[var(--ink2)]';
  return (
    <div className="border border-[var(--line)] rounded-xl p-5 hover:shadow-sm transition-shadow">
      <div className="flex items-start justify-between mb-3">
        <div>
          <h3 className="text-sm font-semibold text-[var(--ink)]">{model.name}</h3>
        ...
        <button
          ...
          className="px-3 py-1 text-xs font-medium text-[var(--ink)] bg-[rgba(10,22,40,0.08)] hover:bg-[rgba(10,22,40,0.08)] rounded-lg transition-colors"
        >
          Configure
        </button>
      </div>
      <div className="grid grid-cols-3 gap-3">
        <div>
          <p className="text-xs font-medium text-[var(--muted)]">Temperature</p>
          <p className="text-xs text-[var(--ink2)] mt-0.5 font-mono">{model.temperature}</p>
        </div>
        <div>
          <p className="text-xs font-medium text-[var(--muted)]">Max Tokens</p>
          <p className="text-xs text-[var(--ink2)] mt-0.5 font-mono">{model.maxTokens.toLocaleString()}</p>
        </div>
        <div>
          <p className="text-xs font-medium text-[var(--muted)]">Cost / 1K tok</p>
          <p className="text-xs text-[var(--ink2)] mt-0.5 font-mono">${model.costPer1kTokens.toFixed(4)}</p>
        </div>
      </div>
      <div className="mt-3 flex items-center gap-2">
        <div className={`w-2 h-2 rounded-full ${model.enabled ? 'bg-emerald-500' : 'bg-[var(--line)]'}`} />
        <span className="text-xs text-[var(--muted)]">{model.enabled ? 'Enabled' : 'Disabled'}</span>
        {model.avgLatencyMs > 0 && (
          <span className="text-xs text-[var(--muted)] ml-auto">~{model.avgLatencyMs}ms</span>
        )}
      </div>
    </div>
  );
...
function UsageChartRow({ log }: { log: UsageLog }) {
  ...
  return (
    <tr className="hover:bg-[var(--paper2)]">
      <td className="px-4 py-3 text-xs text-[var(--ink2)]">{new Date(log.createdAt).toLocaleString()}</td>
      <td className="px-4 py-3 text-xs font-medium text-[var(--ink)]">{log.modelName}</td>
      <td className="px-4 py-3 text-xs text-[var(--ink2)] font-mono">{log.inputTokens.toLocaleString()}</td>
      <td className="px-4 py-3 text-xs text-[var(--ink2)] font-mono">{log.outputTokens.toLocaleString()}</td>
      <td className="px-4 py-3 text-xs text-[var(--ink2)] font-mono">{totalTokens.toLocaleString()}</td>
      <td className="px-4 py-3 text-xs text-[var(--ink2)] font-mono">${costDisplay}</td>
      <td className="px-4 py-3 text-xs text-[var(--ink2)] font-mono">{log.latencyMs}ms</td>
    </tr>
  );
}
  ...
  return (
    <div className="bg-white rounded-xl border border-[rgba(10,22,40,0.15)] p-6 shadow-sm">
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Template Name</label>
          <input
            type="text"
            value={name}
        ...
        <div>
          <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Model</label>
          <select
            value={modelId}
        ...
        <div>
          <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Prompt Template</label>
          <textarea
            value={prompt}
        ...
        <div>
          <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Variables</label>
          <div className="flex flex-wrap gap-2 mb-2">
            {variables.map(v => (
              <span key={v} className="px-2 py-1 bg-[rgba(10,22,40,0.08)] text-[var(--ink)] rounded text-xs font-mono flex items-center gap-1">
                {`{{${v}}}`}
                <button onClick={() => setVariables(prev => prev.filter(x => x !== v))} className="text-[var(--ink2)] hover:text-[var(--ink)] ml-1">
                  x
                </button>
              </span>
            ))}
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-lg border border-[var(--line)] px-3 py-2 text-sm focus:ring-2 focus:ring-[#1E2A3B] focus:border-[var(--ink2)]"
            />
            <button onClick={addVar} className="px-3 py-2 text-sm text-[var(--ink)] hover:bg-[rgba(10,22,40,0.08)] rounded-lg transition-colors">
              Add
            </button>
          </div>
        </div>
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--ink)]">AI / ML Settings</h1>
          <p className="text-[var(--muted)] mt-1">Model selection, prompt templates, and usage monitoring</p>
        </div>
        <button
      ...
      {activeTab === 'models' && (
        <div>
          {editingModel && (
            <div className="bg-white rounded-xl border border-[rgba(10,22,40,0.15)] p-6 mb-6 shadow-sm">
              <h3 className="text-base font-semibold text-[var(--ink)] mb-6">Configure {editingModel.name}</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Temperature</label>
                  <input
                    type="range"
                    min={0}
                    max={2}
                  ...
                  />
                  <p className="text-xs text-[var(--muted)] mt-1 text-center">{editingModel.temperature}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Max Tokens</label>
                  <input
                    type="number"
              ...
              <div className="flex gap-3 mt-6">
                <button
                  ...
                  className="px-5 py-2.5 bg-[var(--ink)] text-white text-sm font-semibold rounded-xl hover:bg-[var(--ink)] transition-colors"
                >
                  Save
                </button>
                <button onClick={() => setEditingModel(null)} className="px-5 py-2.5 text-sm font-medium text-[var(--ink2)] hover:bg-[var(--paper2)] rounded-xl transition-colors">
                  Cancel
                </button>
              </div>
            </div>
          ...
          <Section title="AI Models" description="Configure available AI models and their parameters">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {models.length === 0 && (
                <p className="col-span-3 text-sm text-[var(--muted)] text-center py-8">No models configured. Add models via the API.</p>
              )}
            </div>
          </Section>
        </div>
      ...
      {activeTab === 'templates' && (
        <div>
          ...
          <Section title="Prompt Templates" description="Manage reusable prompt templates with variable substitution">
            {templates.length === 0 ? (
              <p className="text-sm text-[var(--muted)] text-center py-8">No templates yet. Create one above.</p>
            ) : (
              <div className="space-y-3">
                {templates.map(t => {
                  const model = models.find(m => m.id === t.modelId);
                  return (
                    <div key={t.id} className="border border-[var(--line)] rounded-lg p-4 hover:bg-[var(--paper2)] transition-colors">
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1 mt-2">
                            {t.variables.map(v => (
                              <span key={v} className="px-1.5 py-0.5 bg-[rgba(10,22,40,0.08)] text-[var(--ink)] text-xs rounded font-mono">{`{{${v}}}`}</span>
                            ))}
                            {model && (
                              <span className="px-1.5 py-0.5 bg-[var(--paper2)] text-[var(--ink2)] text-xs rounded">{model.name}</span>
                            )}
                          </div>
                        </div>
                        <div className="flex gap-2">
                          <button onClick={() => setEditingTemplate(t)} className="px-3 py-1.5 text-xs font-medium text-[var(--ink)] bg-[rgba(10,22,40,0.08)] hover:bg-[rgba(10,22,40,0.08)] rounded-lg transition-colors">
                            Edit
                          </button>
                        </div>
                      </div>
        ...
        <div>
          {usageSummary && (
            <Section title="Usage Summary" description="Current billing period overview">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  <p className="text-xs font-medium text-[var(--muted)] uppercase tracking-wide">Total Requests</p>
                  <p className="text-xl font-semibold text-[var(--ink)] mt-1">{usageSummary.totalRequests.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-[var(--muted)] uppercase tracking-wide">Input Tokens</p>
                  <p className="text-xl font-semibold text-[var(--ink)] mt-1">{usageSummary.totalInputTokens.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-[var(--muted)] uppercase tracking-wide">Output Tokens</p>
                  <p className="text-xl font-semibold text-[var(--ink)] mt-1">{usageSummary.totalOutputTokens.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-[var(--muted)] uppercase tracking-wide">Est. Cost</p>
                  <p className="text-xl font-semibold text-[var(--ink)] mt-1">${(usageSummary.totalCostCents / 100).toFixed(2)}</p>
                </div>
              </div>
            </Section>
          )}
          <div className="bg-white rounded-xl border border-[var(--line)]">
            <div className="px-6 py-4 border-b border-[var(--line)]">
              <h3 className="text-sm font-semibold text-[var(--ink)]">Recent Usage Logs</h3>
            </div>
            {usageLogs.length === 0 ? (
              <div className="px-6 py-8 text-center text-sm text-[var(--muted)]">No usage logs yet.</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-[var(--line)]">
                  <thead className="bg-[var(--paper2)]">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-medium text-[var(--muted)] uppercase">Time</th>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-[var(--muted)] uppercase">Total</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-[var(--muted)] uppercase">Cost</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-[var(--muted)] uppercase">Latency</th>
                    </tr>
                  </thead>
