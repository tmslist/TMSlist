  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--ink)]">Mobile App Configuration</h1>
          <p className="text-[var(--muted)] mt-1">Manage iOS and Android app settings, versions, and feature rollouts</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <div className="bg-white rounded-xl border border-[var(--line)] p-5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <p className="text-sm font-medium text-[var(--muted)]">iOS App</p>
              <p className="text-lg font-semibold text-[var(--ink)]">{versions.find(v => v.platform === 'ios' && v.status === 'active')?.version || 'N/A'}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[var(--muted)]">Build {versions.find(v => v.platform === 'ios')?.buildNumber || 'N/A'}</span>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-[var(--line)] p-5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <p className="text-sm font-medium text-[var(--muted)]">Android App</p>
              <p className="text-lg font-semibold text-[var(--ink)]">{versions.find(v => v.platform === 'android' && v.status === 'active')?.version || 'N/A'}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[var(--muted)]">Build {versions.find(v => v.platform === 'android')?.buildNumber || 'N/A'}</span>
          </div>
        </div>
      </div>
      ...
      {activeTab === 'versions' && (
        <div>
          <div className="flex justify-between mb-4">
            <h2 className="text-lg font-semibold text-[var(--ink)]">App Versions</h2>
            <button
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {versions.map(version => (
              <div key={version.id} className="bg-white rounded-xl border border-[var(--line)] p-5">
                ...
                <div className="space-y-1 mb-3">
                  <p className="text-lg font-bold text-[var(--ink)]">{version.version}</p>
                  <p className="text-xs text-[var(--muted)]">Build {version.buildNumber} &middot; Min iOS {version.minOsVersion}</p>
                  <p className="text-xs text-[var(--muted)]">Released {new Date(version.releaseDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                </div>
                <p className="text-sm text-[var(--ink2)] bg-[var(--paper2)] rounded-lg p-2.5">{version.releaseNotes}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {activeTab === 'features' && (
        <div>
          <div className="mb-4">
            <h2 className="text-lg font-semibold text-[var(--ink)]">Feature Toggles per Platform</h2>
            <p className="text-sm text-[var(--muted)]">Control which features are enabled for iOS and Android users</p>
          </div>
          ...
          <div className="bg-white rounded-xl border border-[var(--line)] overflow-hidden">
            <table className="min-w-full divide-y divide-[var(--line)]">
              <thead className="bg-[var(--paper2)]">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-[var(--muted)] uppercase">Feature</th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-[var(--muted)] uppercase">iOS</th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-[var(--muted)] uppercase">Android</th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-[var(--muted)] uppercase">Rollout %</th>
                  <th className="px-6 py-3"></th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--line)]">
                {features.map(feature => (
                  <tr key={feature.id} className="hover:bg-[var(--paper2)]">
                    <td className="px-6 py-4">
                      <p className="text-sm font-semibold text-[var(--ink)]">{feature.label}</p>
                      <p className="text-xs text-[var(--muted)] mt-0.5">{feature.description}</p>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <button
                        onClick={() => handleToggleFeature(feature, 'ios')}
                        className={`w-12 h-6 rounded-full transition-colors ${
                          feature.enabledIOS ? 'bg-[var(--ink)]' : 'bg-[var(--paper2)]'
                        }`}
                      >
                    ...
                    <td className="px-6 py-4 text-center">
                      <button
                        ...
                        className={`w-12 h-6 rounded-full transition-colors ${
                          feature.enabledAndroid ? 'bg-green-600' : 'bg-[var(--paper2)]'
                        }`}
                      >
                        <div className={`w-5 h-5 bg-white rounded-full shadow transition-transform ${
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="w-20 h-1.5 bg-[var(--paper2)] rounded-lg appearance-none cursor-pointer accent-[#0A1628]"
                        />
                        <span className="text-xs font-medium text-[var(--ink2)] w-8">{feature.rolloutPercent}%</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <span className="font-mono text-xs text-[var(--muted)]">{feature.key}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {activeTab === 'push' && (
        <div>
          <div className="mb-6">
            <h2 className="text-lg font-semibold text-[var(--ink)]">Push Notification Settings</h2>
            <p className="text-sm text-[var(--muted)]">Configure which notifications are sent to mobile users</p>
          </div>
          ...
          <div className="bg-white rounded-xl border border-[var(--line)] p-6">
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-[var(--line)]">
              <div>
                <p className="text-sm font-semibold text-[var(--ink)]">Master Toggle</p>
                <p className="text-xs text-[var(--muted)]">Enable or disable all push notifications</p>
              </div>
              <button
                onClick={() => handlePushToggle('enabled')}
                className={`w-14 h-7 rounded-full transition-colors ${
                  pushSettings.enabled ? 'bg-[var(--ink)]' : 'bg-[var(--paper2)]'
                }`}
              >
              ...
              {([
                ...
                <div key={item.key} className="flex items-center justify-between py-2">
                  ...
                  <button
                    ...
                    className={`w-12 h-6 rounded-full transition-colors ${
                      pushSettings.enabled && pushSettings[item.key] ? 'bg-[var(--ink)]' : 'bg-[var(--paper2)]'
                    } disabled:opacity-50`}
                  >
                    <div className={`w-5 h-5 bg-white rounded-full shadow transition-transform ${
      ...
      {activeTab === 'deeplinks' && (
        <div>
          <div className="mb-6">
            <h2 className="text-lg font-semibold text-[var(--ink)]">Deep Link Configuration</h2>
            <p className="text-sm text-[var(--muted)]">Configure URI schemes and paths for app deep linking</p>
          </div>

          <div className="bg-white rounded-xl border border-[var(--line)] p-6 mb-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div>
                <label className="block text-sm font-medium text-[var(--ink2)] mb-1">URL Scheme</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Host</label>
                <input
                  type="text"
            ...
            <div className="mb-2">
              <label className="block text-sm font-medium text-[var(--ink2)] mb-2">Path Mappings</label>
            </div>
                ...
                <div key={key} className="flex items-center gap-3">
                  <div className="flex-1">
                    <input
                      ...
                      value={key}
                      readOnly
                      className="w-full rounded-lg border border-[var(--line)] px-4 py-2 text-sm font-mono bg-[var(--paper2)] text-[var(--ink2)]"
                    />
                  </div>
                  ...
                  <div className="flex-1">
                    <input
                      ...
                      className="w-full rounded-lg border border-[var(--line)] px-4 py-2 text-sm font-mono focus:border-[var(--ink2)] focus:ring-[#1E2A3B]"
                    />
                  </div>
                </div>
              ))}
            ...
            <p className="mt-4 text-xs text-[var(--muted)]">
              Example: <code className="bg-[var(--paper2)] px-1.5 py-0.5 rounded">{deepLinks.scheme}://{deepLinks.host}/clinic</code> opens the clinics screen
            </p>
          </div>
        </div>
      )}
      ...
      {activeTab === 'metadata' && (
        <div>
          <div className="mb-6">
            <h2 className="text-lg font-semibold text-[var(--ink)]">App Store Metadata</h2>
            <p className="text-sm text-[var(--muted)]">Manage app icons, screenshots, and store listing details</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl border border-[var(--line)] p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-base font-semibold text-[var(--ink)]">Apple App Store</h3>
              </div>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-[var(--ink2)] mb-1">App Name</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Subtitle</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--ink2)] mb-1">App Icon URL</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--ink2)] mb-2">Screenshots ({metadata.ios.screenshots.length})</label>
                  <div className="flex gap-2 flex-wrap">
                    {metadata.ios.screenshots.map((s, i) => (
                      <div key={i} className="w-16 h-28 bg-[var(--paper2)] rounded-lg border border-[var(--line)] flex items-center justify-center text-xs text-[var(--muted)] font-mono">
                        {i + 1}
                      </div>
                    ))}
                    <button className="w-16 h-28 border-2 border-dashed border-[var(--line)] rounded-lg flex items-center justify-center text-[var(--muted)] hover:border-[var(--ink2)] hover:text-[var(--ink)] transition-colors">
            ...
            <div className="bg-white rounded-xl border border-[var(--line)] p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-base font-semibold text-[var(--ink)]">Google Play Store</h3>
              </div>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-[var(--ink2)] mb-1">App Name</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--ink2)] mb-2">Screenshots ({metadata.android.screenshots.length})</label>
                  <div className="flex gap-2 flex-wrap">
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-colors ${
                      editingVersion.platform === p
                        ...
                        : 'bg-[var(--paper2)] text-[var(--ink2)] border-[var(--line)]'
                    }`}
                  >
                    {p.toUpperCase()}
                  </button>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Version</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Build</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Min OS</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Release Date</label>
                  <input
                    type="date"
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Release Notes</label>
                <textarea
