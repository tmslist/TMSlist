...
function SimpleBar({ value, max, color }: { value: number; max: number; color: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs font-semibold text-[var(--ink2)] w-8 text-right">{value}</span>
    </div>
  );
}
...
export default function ClinicAnalytics({ clinicId, clinicName, isFeatured }: ClinicAnalyticsProps) {
  ...
  if (error) {
    return (
      <div className="bg-[var(--paper2)] rounded-2xl border border-[var(--line)] p-6 text-center">
        ...
        {!error.includes('Sign in') && (
          <button onClick={fetchAnalytics} className="mt-3 text-xs text-[var(--accent)] font-semibold hover:underline">
            Try again
          </button>
        )}
      </div>
  ...
  if (loading) {
    return (
      <div className="bg-[var(--paper2)] rounded-2xl border border-[var(--line)] p-8 text-center">
        <div className="animate-pulse space-y-3">
          <div className="h-4 bg-[var(--paper2)] rounded w-1/3 mx-auto" />
          <div className="h-8 bg-[var(--paper2)] rounded w-1/2 mx-auto" />
          <div className="h-4 bg-[var(--paper2)] rounded w-1/4 mx-auto" />
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[rgba(10,22,40,0.08)] flex items-center justify-center">
            <svg className="w-5 h-5 text-[var(--accent)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
            </svg>
          </div>
          <div>
            <h3 className="font-bold text-[var(--ink)]">Your Clinic Analytics</h3>
            <p className="text-xs text-[var(--muted)]">Performance for <span className="font-medium">{clinicName}</span></p>
          </div>
        </div>
      ...
      {data.clinic.isFeatured && (
        <div className="flex items-center gap-2 bg-[rgba(10,22,40,0.08)] border border-[rgba(10,22,40,0.1)] rounded-xl px-4 py-3">
      ...
      {data.leadsByDay.length > 0 && (
        <div className="bg-white rounded-xl border border-[var(--line)] p-5 shadow-sm">
          <h4 className="text-xs font-bold text-[var(--ink2)] uppercase tracking-wider mb-4">Enquiries Over Time</h4>
          <div className="space-y-2">
            {data.leadsByDay.slice(-14).map((d) => (
              <div key={d.day} className="flex items-center gap-3">
                <span className="text-xs text-[var(--muted)] w-16 shrink-0">{formatDate(d.day)}</span>
                <div className="flex-1 h-4 bg-[var(--paper2)] rounded overflow-hidden">
                  <div
                    className="h-full bg-[var(--ink2)] rounded transition-all"
                    style={{ width: `${Math.round((d.count / maxLeadsDay) * 100)}%` }}
                  />
                </div>
                <span className="text-xs font-semibold text-[var(--ink2)] w-4 text-right">{d.count}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {data.leadsByType.length > 0 && (
        <div className="bg-white rounded-xl border border-[var(--line)] p-5 shadow-sm">
          <h4 className="text-xs font-bold text-[var(--ink2)] uppercase tracking-wider mb-4">Enquiry Sources</h4>
            ...
            {data.leadsByType.map((t) => (
              <div key={t.type}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs font-semibold text-[var(--muted)]">{t.count}</span>
                </div>
                <SimpleBar value={t.count} max={maxType} color={LEAD_TYPE_COLORS[t.type] || 'bg-[var(--ink2)]'} />
              </div>
            ))}
          </div>
        </div>
      ...
      {data.recentLeads.length > 0 && (
        <div className="bg-white rounded-xl border border-[var(--line)] p-5 shadow-sm">
          <h4 className="text-xs font-bold text-[var(--ink2)] uppercase tracking-wider mb-4">Recent Enquiries</h4>
          <div className="space-y-2">
            {data.recentLeads.slice(0, 5).map((lead) => (
              <div key={lead.id} className="flex items-start gap-3 py-2 border-b border-[var(--line)] last:border-0">
                <div className="w-8 h-8 rounded-full bg-[rgba(10,22,40,0.08)] flex items-center justify-center shrink-0 mt-0.5">
                  <span className="text-xs font-bold text-[var(--accent)]">
                ...
                <span className="text-[10px] text-[var(--muted)] shrink-0">{formatDate(lead.createdAt)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {data.overview.leads.count === 0 && data.overview.reviews.count === 0 && (
        <div className="text-center py-8 bg-[var(--paper2)] rounded-xl border border-[var(--line)]">
          <svg className="w-10 h-10 text-[var(--line)] mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
          </svg>
          <p className="text-sm text-[var(--muted)] font-medium">No data yet for this period</p>
          <p className="text-xs text-[var(--muted)] mt-1">Enquiries and reviews will appear here as patients interact with your listing</p>
        </div>
      )}
