...
const PLAN_DISPLAY: Record<string, { name: string; price: string; color: string }> = {
  free: { name: 'Free', price: '$0/mo', color: 'text-[var(--ink2)]' },
  pro: { name: 'Pro', price: '$18/mo', color: 'text-[var(--accent)]' },
  premium: { name: 'Premium', price: '$30/mo', color: 'text-[var(--accent)]' },
  enterprise: { name: 'Enterprise', price: '$60/mo', color: 'text-[var(--ink)]' },
};
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 animate-in zoom-in-95 duration-200">
        ...
        <div className="text-center mb-6">
          <div className="w-14 h-14 bg-gradient-to-br from-[var(--accent2)] to-[var(--accent)] rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-[rgba(10,22,40,0.1)]">
            <svg className="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
          </div>
          <h2 className="text-xl font-bold text-[var(--ink)] mb-1">Unlock {required.name}</h2>
          <p className="text-sm text-[var(--muted)] leading-relaxed">{hint}</p>
        </div>

        <div className="bg-[var(--paper2)] rounded-2xl p-4 mb-6">
          ...
          <div className="flex items-center gap-2">
            <div className="flex-1 h-1.5 bg-[var(--paper2)] rounded-full overflow-hidden">
              <div className="h-full bg-[var(--accent2)] rounded-full" style={{ width: '25%' }} />
            </div>
        ...
        <div className="space-y-2 mb-6">
          {['analytics', 'leadDashboard', 'featuredPlacement'].filter(f =>
            ...
            <div key={f} className="flex items-center gap-2 text-sm text-[var(--ink2)]">
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-3.5 bg-gradient-to-r from-[var(--accent2)] to-[var(--accent)] hover:from-[var(--accent)] hover:to-[#0A1628] text-white font-bold rounded-xl text-sm transition-all shadow-lg shadow-[rgba(10,22,40,0.1)] disabled:opacity-50"
        >
        ...
        <button
          onClick={onClose}
          className="w-full mt-2 py-2 text-sm text-[var(--muted)] hover:text-[var(--ink2)] font-medium transition-colors"
        >
          Maybe later
        </button>
      </div>
