...
const TYPE_COLORS: Record<string, string> = {
  '301': 'bg-[rgba(10,22,40,0.1)] text-[var(--ink)]',
  '302': 'bg-green-100 text-green-700',
  '307': 'bg-amber-100 text-amber-700',
  '308': 'bg-[rgba(201,101,74,0.1)] text-[var(--warm)]',
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-xl font-semibold text-[var(--ink)]">Redirect Manager</h2>
          <p className="text-sm text-[var(--muted)] mt-0.5">{redirects.length} redirect{redirects.length !== 1 ? 's' : ''} configured</p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={fetchRedirects} className="px-3 py-2 bg-white border border-[var(--line)] rounded-lg text-sm font-medium text-[var(--ink2)] hover:bg-[var(--paper2)] transition-colors flex items-center gap-2">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-[var(--line)]">
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-[var(--muted)]">No redirects configured.</p>
            <button onClick={() => openModal()} className="mt-3 text-[var(--warm)] hover:text-[var(--warm)] text-sm font-medium">Add your first redirect</button>
          </div>
        ) : (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-[var(--paper2)] border-b border-[var(--line)]">
                  <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] uppercase">Source Path</th>
                  <th className="px-4 py-3 text-xs font-semibold text-[var(--muted)] uppercase">Target Path</th>
              ...
              <tbody className="divide-y divide-[var(--line)]">
                {redirects.map(redirect => (
                  <tr key={redirect.id} className="hover:bg-[var(--paper2)] transition-colors">
                    <td className="px-4 py-3 text-sm font-mono text-[var(--ink)]">{redirect.sourcePath}</td>
                    <td className="px-4 py-3 text-sm font-mono text-[var(--ink2)]">{redirect.targetPath}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 text-xs font-medium rounded ${TYPE_COLORS[redirect.type]}`}>{redirect.type}</span>
                    </td>
                    <td className="px-4 py-3">
                      <button onClick={() => handleToggle(redirect)} className={`px-3 py-1 text-xs font-medium rounded-lg transition-colors ${redirect.active ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200' : 'bg-[var(--paper2)] text-[var(--muted)] hover:bg-[var(--paper2)]'}`}>
                        {redirect.active ? 'Active' : 'Inactive'}
                      </button>
                    </td>
                    <td className="px-4 py-3 text-sm text-[var(--muted)]">{formatDate(redirect.createdAt)}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <button onClick={() => openModal(redirect)} className="p-1.5 rounded-lg hover:bg-[var(--paper2)] text-[var(--muted)] hover:text-[var(--ink2)]">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                          </svg>
                        </button>
                        <button onClick={() => handleDelete(redirect)} className="p-1.5 rounded-lg hover:bg-red-50 text-[var(--muted)] hover:text-red-600">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editingRedirect ? 'Edit Redirect' : 'Add Redirect'}>
        <div className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Source Path *</label>
            <input type="text" value={sourcePath} onChange={e => handleSourceChange(e.target.value)} placeholder="/old-page" className="w-full rounded-lg border border-[var(--line)] px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-[rgba(10,22,40,0.2)] focus:border-[rgba(201,101,74,0.2)]" />
            <p className="text-xs text-[var(--muted)] mt-1">The path users will be redirected from</p>
          </div>

          <div>
            <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Target Path *</label>
            <input type="text" value={targetPath} onChange={e => setTargetPath(e.target.value)} placeholder="/new-page" className="w-full rounded-lg border border-[var(--line)] px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-[rgba(10,22,40,0.2)] focus:border-[rgba(201,101,74,0.2)]" />
            <p className="text-xs text-[var(--muted)] mt-1">The path users will be redirected to</p>
          </div>

          <div>
            <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Redirect Type</label>
            <select value={type} onChange={e => setType(e.target.value as typeof type)} className="w-full rounded-lg border border-[var(--line)] px-3 py-2 text-sm bg-white focus:ring-2 focus:ring-[rgba(10,22,40,0.2)] focus:border-[rgba(201,101,74,0.2)]">
