  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Given', count: given.length, color: 'text-[var(--ink)]' },
          { label: 'Received', count: received.length, color: 'text-[var(--warm)]' },
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          className="px-4 py-2 rounded-lg text-sm font-medium text-white bg-[var(--ink)] hover:bg-[var(--ink)] transition-colors"
        >
          + New Referral
        </button>
      </div>
      ...
      {currentList.length === 0 ? (
        <div className="bg-white rounded-xl border border-[var(--line)] p-8 text-center">
          <p className="text-[var(--muted)] text-sm">No {activeTab === 'given' ? 'referrals given' : 'referrals received'} yet</p>
        </div>
      ) : (
        <div className="space-y-4">
          {currentList.map(ref => (
            <div key={ref.id} className="bg-white rounded-xl border border-[var(--line)] p-5 shadow-sm">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${
                    activeTab === 'given' ? 'bg-[rgba(10,22,40,0.1)] text-[var(--ink)]' : 'bg-[rgba(201,101,74,0.1)] text-[var(--warm)]'
                  }`}>
                    {activeTab === 'given' ? '→' : '←'}
                  </div>
                  <div>
                    <p className="text-xs text-[var(--muted)] uppercase tracking-wider">Referral</p>
                    <p className="text-sm font-medium text-[var(--ink)]">{ref.referredDoctorId ? `Doctor ID: ${ref.referredDoctorId.slice(0, 8)}...` : 'No doctor specified'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`text-xs px-2.5 py-1 rounded-full font-semibold capitalize ${STATUS_COLORS[ref.status] || 'bg-[var(--paper2)] text-[var(--ink2)]'}`}>
                    {STATUS_LABELS[ref.status] || ref.status}
                  </span>
                  <span className="text-xs text-[var(--muted)]">{new Date(ref.createdAt).toLocaleDateString()}</span>
                </div>
              </div>

              {ref.notes && (
                <div className="bg-[var(--paper2)] rounded-lg p-3 mb-3">
                  <p className="text-xs font-semibold text-[var(--muted)] uppercase tracking-wider mb-1">Notes</p>
                  <p className="text-sm text-[var(--ink2)]">{ref.notes}</p>
                </div>
              )}

              {/* Edit notes */}
              {editingId === ref.id ? (
                <div className="bg-[var(--paper2)] border border-[var(--line)] rounded-lg p-3">
                  <textarea
                    value={editNotes}
              ...
              <div className="flex gap-2 mt-3">
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg text-xs font-medium text-[var(--ink2)] border border-[var(--line)] hover:bg-[var(--paper2)] ml-auto"
                >
                  Edit Notes
                </button>
              </div>
      ...
      {showNewModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            <h2 className="text-lg font-semibold text-[var(--ink)] mb-4">New Referral</h2>
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full px-3 py-2 border border-[var(--line)] rounded-lg text-sm focus:outline-none focus:border-[rgba(10,22,40,0.2)] focus:ring-2 focus:ring-[rgba(10,22,40,0.2)]"
                  placeholder="Referral details, patient info, reason for referral..."
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Initial Status</label>
                <select
                  value={newStatus}
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                disabled={updating}
                className="flex-1 px-4 py-2 rounded-lg text-sm font-medium text-white bg-[var(--ink)] hover:bg-[var(--ink)] disabled:opacity-50"
              >
                Create Referral
              </button>
              <button
