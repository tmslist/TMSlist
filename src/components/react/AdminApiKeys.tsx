  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--ink)]">API Keys</h1>
          <p className="text-[var(--muted)] mt-1">Manage access tokens for third-party integrations</p>
        </div>
        <button
      ...
      {newKey.name !== undefined && !newKey.permissions.length === false && (
        <div className="bg-white rounded-xl border border-[rgba(10,22,40,0.15)] p-6 mb-6 shadow-sm ring-1 ring-[rgba(10,22,40,0.1)]">
          <h3 className="text-base font-semibold text-[var(--ink)] mb-4">Create New API Key</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Key Name</label>
              <input
                type="text"
                value={newKey.name}
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--ink2)] mb-1">Expires At (optional)</label>
              <input
                type="date"
          ...
          <div className="mb-4">
            <label className="block text-sm font-medium text-[var(--ink2)] mb-2">Permissions</label>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {PERMISSIONS.map(p => (
                <label key={p} className="flex items-center gap-2 px-3 py-2 rounded-lg border border-[var(--line)] text-sm cursor-pointer hover:bg-[var(--paper2)]">
                  <input
                    type="checkbox"
                  ...
                  />
                  <span className="font-medium text-[var(--ink2)] text-xs">{p}</span>
                </label>
              ))}
            </div>
          </div>
        ...
        <table className="min-w-full divide-y divide-[var(--line)]">
          <thead className="bg-[var(--paper2)]">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-[var(--muted)] uppercase">Key Preview</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-[var(--muted)] uppercase">Permissions</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-[var(--muted)] uppercase">Expires</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-[var(--muted)] uppercase">Last Used</th>
          ...
          <tbody className="divide-y divide-[var(--line)]">
            {keys.map(key => (
              <tr key={key.id} className="hover:bg-[var(--paper2)]">
                <td className="px-6 py-4 text-sm font-semibold text-[var(--ink)]">{key.name}</td>
                <td className="px-6 py-4 font-mono text-xs text-[var(--muted)]">{key.keyPreview}</td>
                <td className="px-6 py-4">
                  <div className="flex flex-wrap gap-1">
                    {key.permissions.map(p => (
                      <span key={p} className="px-2 py-0.5 bg-[rgba(10,22,40,0.08)] text-[var(--ink)] text-xs rounded-md font-medium">{p}</span>
                    ))}
                  </div>
                </td>
                <td className="px-6 py-4 text-sm text-[var(--ink2)]">{formatDate(key.expiresAt)}</td>
                <td className="px-6 py-4 text-sm text-[var(--ink2)]">{formatDate(key.lastUsedAt)}</td>
                <td className="px-6 py-4">
                  <button
                    onClick={() => toggleKey(key)}
                    className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${
                      key.active ? 'bg-emerald-100 text-emerald-700' : 'bg-[var(--paper2)] text-[var(--muted)]'
                    }`}
                  >
            ...
            {keys.length === 0 && (
              <tr><td colSpan={7} className="px-6 py-12 text-center text-[var(--muted)]">No API keys yet. Create one above.</td></tr>
            )}
          </tbody>
        </table>
      </div>
