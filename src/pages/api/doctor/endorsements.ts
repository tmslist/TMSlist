...
import { db } from '../../../db';
import { doctors, doctorEndorsements, users, auditLog } from '../../../db/schema';
...
export const GET: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const endorsements = await db.select().from(doctorEndorsements)
      .where(eq(doctorEndorsements.toDoctorId, doctorId))
      .orderBy(desc(doctorEndorsements.createdAt));
    return json({ endorsements });
  } catch (err) { console.error(err); return json({ error: 'Failed to load endorsements' }, 500); }
};
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const { toDoctorId, condition, note } = body;
    if (!toDoctorId || !condition) return json({ error: 'toDoctorId and condition are required' }, 400);
    let doctorId: string;
    if ('doctorId' in doctorCtx) doctorId = doctorCtx.doctorId;
    else return json({ error: 'Doctor not found' }, 404);
    const result = await db.insert(doctorEndorsements).values({
      fromDoctorId: doctorId,
      toDoctorId,
      condition,
      note: note ?? null,
    }).returning();
    await logAudit(doctorCtx.userId, 'create', 'doctor_endorsement', result[0].id);
    return json(result[0], 201);
  } catch (err) { console.error(err); return json({ error: 'Failed to create endorsement' }, 500); }
};
