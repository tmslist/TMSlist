...
import { db } from '../../../db';
import { doctors, doctorMessages, users, auditLog } from '../../../db/schema';
...
export const GET: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const messages = await db.select().from(doctorMessages)
      .where(eq(doctorMessages.doctorId, doctorId))
      .orderBy(desc(doctorMessages.createdAt));
    return json({ messages });
  } catch (err) { console.error(err); return json({ error: 'Failed to load messages' }, 500); }
};
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const { patientEmail, subject, body: messageBody } = body;
    if (!patientEmail || !messageBody) return json({ error: 'patientEmail and body are required' }, 400);
    let doctorId: string;
    if ('doctorId' in doctorCtx) doctorId = doctorCtx.doctorId;
    else return json({ error: 'Doctor not found' }, 404);
    const result = await db.insert(doctorMessages).values({
      doctorId,
      senderId: doctorCtx.userId,
      patientEmail,
      subject: subject ?? null,
      body: messageBody,
      isRead: false,
    }).returning();
    await logAudit(doctorCtx.userId, 'create', 'doctor_message', result[0].id);
    return json(result[0], 201);
  } catch (err) { console.error(err); return json({ error: 'Failed to send message' }, 500); }
};
...
export const PUT: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    await db.update(doctorMessages)
      .set({ isRead: true })
      .where(and(eq(doctorMessages.id, id), eq(doctorMessages.doctorId, doctorId)));
    await logAudit(doctorCtx.userId, 'mark_read', 'doctor_message', id);
    return json({ success: true });
  } catch (err) { console.error(err); return json({ error: 'Failed to update message' }, 500); }
};
