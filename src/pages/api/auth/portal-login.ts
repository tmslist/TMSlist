...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    // Doctor portal allows ANY email (no ADMIN_EMAILS check)
    ...
    await resend.emails.send({
      ...
      to: email,
      subject: 'Your TMS List Doctor Portal login link',
      html: `
        <div style="font-family: system-ui, -apple-system, sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 20px;">
          <div style="text-align: center; margin-bottom: 32px;">
            ...
            <p style="color: #059669; margin-top: 8px; font-weight: 600;">Doctor Portal</p>
          </div>
          <div style="background: #ffffff; border: 1px solid #d1fae5; border-radius: 12px; padding: 32px; text-align: center;">
            <p style="color: #374151; font-size: 16px; margin: 0 0 24px;">Click the button below to sign in to your Doctor Portal. This link expires in 15 minutes.</p>
            <a href="${magicUrl}" style="display: inline-block; background: #059669; color: #ffffff; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 16px;">Sign In to Portal</a>
            <p style="color: #9ca3af; font-size: 13px; margin-top: 24px;">If you didn't request this link, you can safely ignore this email.</p>
          </div>
          <p style="color: #9ca3af; font-size: 12px; text-align: center; margin-top: 24px;">Manage your clinic listing, view reviews, and track enquiries.</p>
        </div>
      `,
    });
