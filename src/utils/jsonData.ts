...
export async function getTopMetros(minClinics = 1): Promise<MetroArea[]> {
  const cityMap = new Map<string, MetroArea>();

  for (const c of getStore().allRows) {
    if (c.country.toUpperCase() !== 'US' || !c.verified) continue;

    const key = `${c.city}|${c.state}`;
    if (!cityMap.has(key)) {
      const stateCode = c.state?.toUpperCase() || '';
      cityMap.set(key, {
        citySlug: (c.city || '').toLowerCase().replace(/\s+/g, '-'),
        cityName: c.city || 'Unknown',
        stateCode,
        stateSlug: (STATE_NAMES[stateCode] || stateCode).toLowerCase().replace(/\s+/g, '-'),
        stateName: STATE_NAMES[stateCode] || stateCode,
        clinicCount: 0,
      });
    }
    cityMap.get(key)!.clinicCount++;
  }

  return [...cityMap.values()]
    .filter(m => m.clinicCount >= minClinics)
    .sort((a, b) => b.clinicCount - a.clinicCount || a.cityName.localeCompare(b.cityName));
}
